...
class OpenAILiveTurn(BaseLiveTurn):
    ...
    async def _send_audio_stream(self) -> None:
        """Feed the session's audio timeline at the room's rate.

        Live advances that timeline off the audio it is handed, so the
        quiet this synthesizes between mic frames has to leave at 1x or
        the server's silence-driven endpointing runs fast. Only the
        newest queued frame is that live edge: anything stacked behind it
        was captured earlier — while the socket was still dialling, or
        behind a slow tick — and is already that old in the room, so it
        owes no wall clock and goes out as fast as the socket takes it.
        """
