"""jasper-satellite-onboard — Pi-mediated provisioning for the JTS
Waveshare AMOLED satellite. Mirror of jasper-satellite-onboard for the
satellite hardware path; the two are deliberately near-identical so
the future web wizard can shell out to either by device-class.

Plug a fresh satellite into a Pi USB-C port and run this command.
The script:

  1. Finds the satellite on USB CDC (/dev/ttyACM* matching ESP32-S3
     VID:PID)
  2. (Optional) Flashes the AMOLED satellite firmware via esptool
  3. Reads the Pi's current WiFi credentials (NetworkManager or
     wpa_supplicant)
  4. Pushes them via Improv-over-Serial — same protocol the firmware's
     ImprovWiFi instance accepts on USB-CDC
  5. Watches for the satellite to come up on the network (ping mDNS
     `jasper-satellite-amoled.local`)

Improv-over-Serial protocol reference:
  https://www.improv-wifi.com/serial/

The satellite keeps WiFi creds in NVS flash, so subsequent boots
reconnect without intervention. Re-run this script after a network
change to push new creds — the satellite accepts SUBMIT_SETTINGS at
any time.
"""
...
# generic S3 PID. The Waveshare AMOLED-1.8 satellite uses the same
# descriptor as any other ESP32-S3 dev board.
ESP32_S3_VID = 0x303A
...
def find_satellite(explicit_port: str | None = None) -> Satellite:
    """Locate the satellite's USB CDC serial port. Raises if not found."""
    if explicit_port:
        if not Path(explicit_port).exists():
            raise RuntimeError(f"explicit port not found: {explicit_port}")
        return Satellite(port=explicit_port, vid=0, pid=0)
    ...
    if not matches:
        raise RuntimeError(
            "No ESP32-S3 satellite found on USB. Plug it into the Pi and try again. "
    ...
    p = matches[0]
    return Satellite(port=p.device, vid=p.vid, pid=p.pid)
...
# Boot-log signature emitted by the JTS satellite firmware in setup().
# See firmware/satellite-amoled/src/main.cpp's `Serial.println("[boot] jasper-satellite
...
_SATELLITE_BOOT_SIGNATURE = b"jasper-satellite-amoled firmware"


def probe_jts_satellite(port: str, *, timeout_s: float = 5.0) -> bool:
    """Identify whether the device on `port` is running our satellite
    ...
    behavior is on the ROM side, not pyserial's). The satellite's setup()
    then prints `[boot] jasper-satellite-amoled firmware v<version>` over Serial
    within ~1-2 seconds. We open, listen for that exact string, and
    return True if seen.

    Why this beats sending an Improv RPC: the satellite's `tryConnectStored`
    ...
    Why opening the port at all is acceptable: on the JTS satellite path
    `--auto` mode runs an mDNS pre-check first (jasper-satellite-amoled.local in
    3s); already-online satellites short-circuit before this probe runs,
    so the chip-reset disruption only fires when the satellite wasn't
    ...
    # the probe sees nothing even on a real JTS satellite. Reset is the
    # signal here, not the bug.
    try:
        ser = serial.Serial(port, 115200, timeout=0.2)
    except (OSError, Exception) as e:  # noqa: BLE001
        logger.debug("satellite probe: open %s failed: %s", port, e)
        return False
    ...
    try:
        ...
        while time.monotonic() < deadline:
            try:
                ...
                logger.debug("satellite probe: read failed: %s", e)
                return False
            if chunk:
                buf.extend(chunk)
                if _SATELLITE_BOOT_SIGNATURE in buf:
                    return True
            else:
                time.sleep(0.05)
        return False
...
def push_credentials(port: str, ssid: str, password: str, *, timeout: float = 30.0) -> None:
    """Open the satellite's serial port and run the Improv handshake.

    Walks the satellite through PROVISIONING → PROVISIONED, raising on error
    or timeout. The satellite library validates the SSID/password by actually
    connecting to WiFi before reporting PROVISIONED, so a successful
    return means the satellite is on WiFi."""
    import serial  # pyserial; deferred so import is optional for tests
            ...
            if chunk:
                ...
                for pkt_type, data in _scan_packets(buf):
                    if pkt_type == IMPROV_PKT_CURRENT_STATE and len(data) == 1:
                        state = data[0]
                        logger.info("satellite reports state=0x%02x", state)
                        ...
                        if data[0] != IMPROV_ERROR_NONE:
                            raise RuntimeError(
                                f"Improv error from satellite: 0x{data[0]:02x}"
                            )
                ...
                time.sleep(0.05)
        raise TimeoutError(
            f"Satellite did not reach PROVISIONED state within {timeout:.0f}s. "
            "Check the satellite's WS2812 status LED — red blink means WiFi "
...
def flash_firmware(port: str, bin_path: Path) -> None:
    ...
    doesn't exist — onboarding is still useful when the satellite is already
...
def wait_for_online(hostname: str, timeout: float = 30.0) -> str | None:
    """Resolve `hostname` (e.g. jasper-satellite-amoled.local) until it answers, or
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--bin", default="/opt/jasper/firmware/satellite-amoled/jasper-satellite-amoled.bin", type=Path,
        help="firmware bin to flash (skipped if missing — for already-flashed satellites)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="force-flash even if the satellite is already running JTS firmware "
    ...
    parser.add_argument(
        "--mdns-host", default="jasper-satellite-amoled.local",
        help="mDNS hostname to ping after provisioning (default jasper-satellite-amoled.local)",
    )
    parser.add_argument(
        "--auto", action="store_true",
        help="udev-triggered idempotent mode: if the satellite already responds at "
    ...
    # if the satellite already resolves on mDNS we treat that as "already
    # provisioned and on the same WiFi", and exit without touching it.
    # Opening the serial port to talk Improv would reset the satellite via
    ...
    if args.auto:
        ...
        if ip is not None:
            logger.info(
                "satellite already online at %s (%s) — auto mode short-circuit, "
                "no flash or cred push needed",
                args.mdns_host, ip,
            )
            return 0
        logger.info(
            "satellite not yet online at %s — proceeding with onboard flow",
            args.mdns_host,
        )

    try:
        satellite = find_satellite(args.port)
    except RuntimeError as e:
        logger.error(str(e))
        return 1
    logger.info("found satellite on %s (vid=0x%04x pid=0x%04x)", satellite.port, satellite.vid, satellite.pid)
    ...
    # S3 board" to "this S3 board is running our satellite firmware".
    is_jts_satellite = probe_jts_satellite(satellite.port)
    if is_jts_satellite:
        logger.info(
            "boot-log probe succeeded — device is running JTS satellite "
            "firmware, no flash needed",
        )
    else:
        logger.info(
            "boot-log probe didn't see JTS firmware signature — "
            "device either needs flashing, or it's not a JTS satellite",
        )

    # Flash decision matrix:
    #   is_jts_satellite    args.flash  args.no_flash  args.auto  → flash?
    #   ─────────────  ──────────  ─────────────  ─────────  ────────
    ...
    # never shows, even on a real JTS satellite. Falling through to push in
    # --auto mode means: if it IS our satellite that the probe missed, the
    ...
    if is_jts_satellite:
        ...
        if args.no_flash:
            ...
            logger.info(
                ...
                "satellite the push will succeed, otherwise it'll time out "
    ...
    try:
        push_credentials(satellite.port, ssid, password)
    except (RuntimeError, TimeoutError) as e:
        logger.error("Improv push failed: %s", e)
        return 4

    logger.info("satellite reports PROVISIONED — checking mDNS %s", args.mdns_host)
    ip = wait_for_online(args.mdns_host, timeout=20.0)
    if ip is None:
        speaker_host = os.environ.get("JASPER_HOSTNAME", "jts.local")
        logger.warning(
            "satellite did not appear at %s within 20 s. WiFi probably worked "
            "(Improv waits for it before reporting PROVISIONED), but mDNS "
            "may be filtered on your network. The satellite should still reach "
            "%s for HTTP control.", args.mdns_host, speaker_host,
        )
    else:
        logger.info("satellite online at %s (%s)", args.mdns_host, ip)
