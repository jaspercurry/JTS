...
"""A round's near-field driver takes, band by band, with each driver's distance self-test."""
...
def _cmd_nearfield(args: argparse.Namespace) -> int:
    ...
    document = nearfield_view((take for row in view_sets(manifest) for take in row["takes"]),
...
def add_parser(sub: argparse._SubParsersAction) -> None:
    parser = sub.add_parser(
        "nearfield", help="each near-field take band by band, and each driver's step between distances against a piston",
    )
