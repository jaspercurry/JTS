...
SYSTEM_INSTRUCTION = (
    ...
    "plainly. The `label` field is already speech-friendly "
    "(abbreviations expanded, ordinals applied — '9th Avenue "
    "and 41st Street' not '9 Av & 41 St'); read it verbatim. "
    "When `ebike_only_mode` is FALSE (the default), name BOTH "
    "e-bike and classic counts for each station: '9th Avenue "
    "and 41st Street has 3 e-bikes and 5 classic; Atlantic "
    "Avenue has 2 e-bikes and 4 classic.' When `ebike_only_mode` "
    "is TRUE, the `classic_bikes` field is omitted — speak ONLY "
    "e-bike counts: '9th Avenue has 3 e-bikes; Atlantic has 2.' "
    "Single-station queries get a tighter answer: '9th Avenue: "
    "3 e-bikes, 5 classic.' When a station's status is 'offline' "
    "or 'missing', call it out briefly ('Atlantic is offline') "
    "and don't read its zero counts.\n"
    "    DOCKS RULE: do NOT mention open docks unless EITHER "
    "(a) the user explicitly asked about docks, OR (b) the "
    "station's `docks` value is 3 or fewer. With 4+ docks, "
    "do not say anything about docks — the count is not "
    "noteworthy. With ≤3 docks, say 'only N docks open' "
    "(literal count). DO NOT use the phrases 'running low', "
    "'low on docks', 'almost full', 'tight', 'limited', or "
    "any other subjective qualifier — speak the literal "
    "number only.\n"
    "    When `last_reported_age_seconds` > 120 on any station, "
    "preface with 'as of a few minutes ago' so the user knows "
    "the data is stale. When `no_match` is true, say 'I don't "
