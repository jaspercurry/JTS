...
schema, parsing, freshness checks, and atomic writes, plus the artifact
builders that turn already-collected observations into a validation
artifact. Those builders read already-exposed runtime status; they do not
play audio, open capture loops, persist chip settings, or mutate audio
services. The operator-controlled program that collects those
observations — XVF chip probing, the passive observation window, and the
`jasper-audio-hw-validate` CLI — lives in `jasper.cli.audio_hw_validate`.
"""
