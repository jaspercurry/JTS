...
`1a86:7523` appears on any USB port, udev passes that event's exact
`/dev/ttyUSB…` path to one systemd job. The helper opens only that path,
verifies product `MT320RUBL40ProV3` in the same controller session, and sends
the vendor stop request. A different CH340 product receives the non-motion
...
the tty or controller is still settling, it retries within the adapter's
attempt limit and the service's time limit. There is no timer or resident
process. Disconnecting the device stops the job; reconnecting starts a new one.
