...
What it does NOT own: the capture-plan shape (`CapturePlan`,
`CapturePlanEntry`), the attempt ceiling, and `CaptureSpecError` live in
:mod:`jasper.capture_protocol` and are imported back here. A session builder
has to state a plan without importing this parked package, so that half of the
contract is owned by neither side of the transport.
