...
``os.environ`` snapshot taken at process start. Two kinds of consumer:
  ...
  * :func:`effective_hostnames` and :func:`configured_hostname` — on the
    ...
    within one reconciler period, rather than answering with the
...
process can read it fresh — a CLI run over ssh has no
``EnvironmentFile=`` at all, and a daemon's copy is frozen at unit
start — so ``jasper.identity`` takes its hostname from here first.
"""
