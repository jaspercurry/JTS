...
def _section_html(group: str, rows: Sequence[NavRow], *, heading: bool) -> str:
    ...
    # A lone section on a hub would repeat the page title, so it is labelled
    # instead of headed.
...
def landing_groups_html(rows: Sequence[NavRow], *, headings: bool = True) -> str:
    """The `<nav class="groups">` inner markup, one section per group."""
    groups: dict[str, list[NavRow]] = {}
    for row in rows:
        groups.setdefault(row.group, []).append(row)
    return "\n\n".join(
        _section_html(g, r, heading=headings) for g, r in groups.items()
    )
