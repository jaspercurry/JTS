...
<section id="placement" data-envelope-section="placement" class="info-card" hidden>
  ...
  <div id="position-prompt" class="note-box" hidden>
    <p class="note-box__lead">Move to position <span id="position-current">2</span> of <span id="position-total">__DEFAULT_ROOM_POSITION_COUNT__</span>.</p>
    <p class="hint hint--mt-sm">Move about 30 cm from the previous position, keep the microphone at ear height, then continue.</p>
  </div>
</section>
...
<section id="capture-setup" data-envelope-section="capture-setup" class="mic-panel" hidden>
  ...
  <div class="mic-grid">
    ...
    <p id="local-input-hint" class="hint hint--flush local-capture-only">Your USB measurement mic should appear automatically. Tap <strong>Refresh microphones</strong> if it doesn’t, then select it before <strong>Allow microphone</strong>.</p>
    ...
    <div id="upload-row" class="mic-row" hidden>
      ...
      <p class="hint hint--flush">Calibration files from miniDSP, Dayton Audio, Cross-Spectrum and the rest of the REW ecosystem describe what the microphone <em>hears</em>, and JTS inverts them. Choose “a correction to add” only if the file’s own documentation says its numbers are already the correction.</p>
    </div>
...
<div id="constraints" hidden aria-live="polite">
...
<section id="run-defaults" data-envelope-section="run-defaults" class="info-card" hidden>
  ...
  <div id="measurement-options" hidden>
    <label for="positions-select">Positions to measure</label>
    <select id="positions-select" form="dummy">
      __ROOM_POSITION_OPTIONS__
    </select>
    <p class="hint hint--mt-sm">More positions describe more of the listening area. We'll guide you through each one.</p>

    <label for="target-select" class="field-label--gap">Target curve</label>
    <select id="target-select" form="dummy">
      __TARGET_PROFILE_OPTIONS__
    </select>

    <label for="strategy-select" class="field-label--gap">Correction strategy</label>
    <select id="strategy-select" form="dummy">
      __CORRECTION_STRATEGY_OPTIONS__
    </select>
    <p class="hint hint--mt-sm">Balanced is the recommended household setting. Safe makes fewer, gentler adjustments.</p>
  </div>
</section>
...
<section id="level-check" data-envelope-section="level-check" class="info-card" hidden>
  ...
  <p id="autolevel-hint" class="hint hint--mt-md">The speaker slowly raises a short test tone until the microphone hears a clear measurement level, then stops automatically. If it sounds comfortably loud first, choose <strong>Lock now</strong>. This takes only a few seconds.</p>
  <p class="hint hint--mt-md">JTS temporarily pauses your current sound settings so it can measure the room clearly. They return unless you apply the new correction.</p>
  <div id="autolevel-status" class="note-box" hidden>
    <p id="autolevel-line" class="note-box__lead">Auto-leveling…</p>
    <p id="autolevel-detail" class="hint hint--mt-sm"></p>
  </div>
</section>
...
<section id="measurement-review" data-envelope-section="measurement-review" hidden>
  <div id="result-section" hidden>
    ...
    <p class="hint">
      <span class="chart-legend-color-measured">red</span> = measured (averaged across positions),
      <span class="chart-legend-color-target">gray dashed</span> = target,
      <span class="chart-legend-color-predicted">green</span> = predicted post-correction.
      <span class="chart-legend-color-filter">blue dashed</span> = filter effect.
      After Verify: <span class="chart-legend-color-verify">purple dashed</span> = post-correction measurement,
      with the measured before→after gap shaded
      <span class="chart-legend-color-predicted">green</span> where it moved toward target and
      <span class="chart-legend-color-regressed">amber</span> where it moved away.
    </p>
...
<section id="result-proof" data-envelope-section="result-proof" hidden></section>
