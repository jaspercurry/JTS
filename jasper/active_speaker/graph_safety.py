...
def bass_extension_block_valid(
    view: GraphView, profile_summary: Mapping[str, Any],
) -> BassExtensionBlockEvidence:
    """The static proof accepts no unvalidated bass block."""
    definitions = tuple(sorted(name for name in view.filters if name.startswith("bass_ext")))
    references = tuple(
        step for step in view.pipeline_steps
        if any(name.startswith("bass_ext") for name in step.names)
    )
    valid = (
        view.parsed_ok and profile_summary.get("authority_valid") is True
        and not profile_summary.get("runtime_block_required")
        and not definitions and not references
    )
    return BassExtensionBlockEvidence(
        valid, False, definitions,
        tuple(sorted({channel for step in references for channel in step.channels})),
