...
class SummedAcousticResult:
    ...
    snr: dict[str, Any] | None = None
    ambient: dict[str, Any] | None = None
    # True when null_depth_db was reduced because the overlap-band SNR could not
    # prove a deeper null. The verdict is always decided from the UNCAPPED depth.
...
def _capture_to_magnitude(
    ...
    *,
    ...
    caller MUST measure its signal side on that same table, since the two are
    subtracted per ``band_id``. A sweep too narrow to cover a canonical band
    needs :func:`~jasper.audio_measurement.snr_policy.sweep_excitation_bands`.
    """
