...
The v2-specific state lives under ``status["crossover_v2"]`` and is never
re-derived here: ``CrossoverV2Session`` owns those decisions, the reason codes
are :mod:`jasper.active_speaker.crossover_v2.refusal_copy`'s, and this module
maps a reason code to its template copy through that module's
``REASON_REGISTRY``.
"""
...
def _closing_envelope(status: Mapping[str, Any]) -> dict[str, Any]:
    ...
    **The confirm belongs to whoever can reach it** (#2881): on a RELAY session
    the phone owns Retake / Continue, and this screen points there; on a WIRED
    session there is no phone, so the same two moves are minted here against the
    endpoints the wired runner already listens on. Both carry
    ``show_during_relay`` because the session is by definition still in flight.
    ...
    alternate_actions: list[dict[str, Any]] = [
        {
            ...
            "body": {},
            # The same escape hatch verify_fail has: a relay still winding down
            # from stage 1 would otherwise blanket-hide every alternate, leaving
            # a household on the decision screen with no way out.
            "show_during_relay": True,
        },
        {
            "id": "review_decline",
...
def _wired_session(status: Mapping[str, Any]) -> bool:
    ...
    Reads the transport the slot published (``relay.source``) rather than
    inferring it from an absent ``tap_link``: "no phone link" is also what a
    relay session looks like for the second before registration returns.

    False with no session in flight, which is what a caller minting live
    controls wants — an offer outliving the session would be an offer to a 409.
    """
...
def _follow_the_prompts(status: Mapping[str, Any]) -> str:
    ...
    Two measuring screens name the place, and on the WIRED source there is no
    measurement page to name — the walkthrough under the live-session block is
    where each spot's prompt renders (#2881).
    """
    return (
...
def _verify_fail_envelope(
    ...
    **A code no retry can clear does not get a "Try again"** (#1873). For every
    ...
    the JS action-row renderer's relay-in-flight gate otherwise blanket-clears
    EVERY alternate while the relay object is still transitioning, so a
    household landing here saw no buttons at all. ``verify_retry`` deliberately
    keeps NO such flag — it starts a brand-new relay session, which is exactly
    the race the gate exists to prevent.
    """
    remeasure = {
        "id": "verify_remeasure",
        "label": "Re-measure",
