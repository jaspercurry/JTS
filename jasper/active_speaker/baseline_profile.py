...
_APPLIED_BASS_EXTENSION = object()
...
def _source_payload(
    ...
    **It is a source fingerprint, not a content hash of the emitted graph.**
    ``candidate_graph_context``'s graph fields — resolved playback device,
    capture device and format, domain, program channel, pair trim — never reach
    here, since that dict is assembled after this call returns (only its
...
def build_baseline_profile_candidate(
    ...
    *,
    ...
    if driver_domain and measured_candidate is None:
        # A bonded follower compiles from the applied box's evidence and has no
        # candidate of its own, so it takes that box's family: the bass owner's
        # subsonic high-pass is driver protection, and bonding must not drop it.
        bass_extension = dict(
            applied_bass_extension_field(load_applied_baseline_profile_state()) or {}
        )
    if preserved_applied_profile is not None:
    ...
    if write:
        ...
        if driver_domain:
            ...
            yaml = emit_active_speaker_driver_domain_config(
                ...
                baseline_id=f"baseline-{_safe_id(topology.topology_id)}",
                bass_extension=bass_extension or None,
            )
        else:
    ...
    payload = {
        ...
        "recomposition_snapshot": {
            ...
            "blend_correction": blend_correction,
            # Layer 2's adopted bass family: the emitter's and the runtime
            # proof's only input. Absent on profiles saved before it existed.
    ...
    if driver_domain:
        ...
        # whole-graph verifier. It comes from the same field the emitter was
        # given, never from filter-name inference or caller I/O.
        payload["bass_extension_profile_summary"] = graph_summary(
            bass_extension or None
        )
    ...
    if not driver_domain and candidate.get("permissions", {}).get("may_apply"):
        ...
        try:
            ...
            graph_proof = classify_bass_extension_graph(
                ...
                desired_bass_extension=applied_bass_extension_field(candidate),
            )
            proof_detail = (
