...
imports. Its filters are persisted and applied on hardware, so read a change
to any term as a change to a live correction profile. The composed depth is
the min of the design doc's three terms plus two optional cloud-derived ones;
every term is narrowing only, and an absent optional term composes to
byte-identical output. Not to be confused with :mod:`jasper.correction.
envelope` or ``crossover_envelope``/``_v2``, which are screen envelopes.
