...
# The second argument is the relay CaptureResult (wav + phone-reported device +
...
# with zero spacing. ``phase`` (keyword-only) is the CONDUCTOR's own flow
# phase (issue #1855) — NOT ``program.phase``. The two are different
...
# every cloud position as "verify". ``Callable[..., R]`` (rather than a
# precise positional list, matching ``jasper.google_creds``'s
# ``service_factory``) because the keyword-only ``phase`` can't be expressed
# in the bracketed positional form.
AnalyzeCapture = Callable[..., ProgramAnalysis]
