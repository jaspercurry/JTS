...
class FlatSpecReport:
    ...
    **"not stated", never "zero"**. ``reference_band_hz`` is the span whose
    ...
    ROOM's floor and its provenance, echoed the way the clamps above are.
    They clamp nothing: unlike ``trusted_floor_hz`` no band edge moves for
    them and no verdict reads them -- what they mark is
    :attr:`BandResult.room_entangled_below_hz`. The default source is
    ``unknown``, which is the state a rig whose reflection finder never
    fires stays in (#3502), and is never to be read as "no room here".
    """
...
def _room_entangled_below_hz(
    ...
    where inside the graded span a gate cannot separate speaker from room. An
    unknown (``None``) or non-finite floor marks nothing, which is "not
    stated" and not "clean" -- see :class:`BandResult`.
    """
    ...
    *,
    ...
    Args:
        ...
        entanglement_floor_source: which of
            ...
            ``smoothing_fraction``: nothing here validates it against the
            floor beside it.
