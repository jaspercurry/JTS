...
Locating the schedule inside a capture uses ``program_analysis``'s private
``_global_offset`` / ``_locate_segments`` / ``_estimate_drift``, deliberately:
the anchors a distortion read rides MUST be the anchors the session's own
analysis used. The durable fix is for a capture to BANK its anchors, and the
fidelity gate is what proves the reproduction landed until it does. The math
is :mod:`jasper.audio_measurement.distortion`'s, called rather than
re-derived.
"""
