...
A CHECK/MEASURE program must be admitted before it may play, and again at play
time from a fresh byte readback
(docs/crossover-measurement-productization-design.md §5.3). Two independent
parts: N per-segment prepared plans through the same closed ledger the isolated
driver capture uses, with the session volume folded into every segment's
effective peak; and per-channel whole-file facts recomputed from the RENDERED
BYTES, which is what makes admission about the artifact rather than the
composer's intent.
