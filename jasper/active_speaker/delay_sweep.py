...
# A reverse null this deep at Fc is a delay answer worth prescribing: below it
# the residual is dominated by something the delay cannot move. Both figures are
# depths in dB re the Fc/2 and 2*Fc shoulder mean, the one null-depth definition
# `analyze_summed_crossover` already emits.
ROBUST_NULL_DEPTH_DB = 20.0
# When NO coordinate in the whole bounded grid reaches this, the residual at Fc
# is not a timing residual at all — it is axis or lobing. Disclosed as a
# verdict, never raised as an error.
...
def reverse_null_graph(
    ...
    *,
    ...
    and pipeline step that is not the named branch's ``Gain.inverted`` bit or
    the named branch's ``Delay`` value is carried through untouched. Re-emitting
    from a preset would reproduce a graph that merely ought to match.

    ``delay_role`` is ``None`` at the zero coordinate, where no branch is
    delayed. The returned graph is runtime-only input: the caller loads it with
    ...
    if delay_role is not None:
        delay_name = driver_delay_name(delay_role)
        ...
        patched_delay["parameters"] = {
            **delay_params,
            # The proof below recomputes this from delay_us through the same
            # single quantizer, so the two can never disagree.
            "delay": quantized_delay_ms(delay_us),
        }
...
def capture_row(
    ...
    *,
    ...
    return {
        ...
        "pose_deg": pose_deg,
        "relative_delay_us": float(relative_delay_us),
    }
...
def _pose_best(
    ...
    """The deepest coordinate this one pose saw, or None when it saw nothing."""

    depths: dict[float, float] = {}
    for delay_us, rows in rows_by_delay.items():
        pose_rows = [row for row in rows if row.get("pose_deg") == pose_deg]
        readings = [
            float(row["acoustic"]["null_depth_db"])
            for row in pose_rows
            if isinstance(row.get("acoustic"), Mapping)
            and isinstance(row["acoustic"].get("null_depth_db"), (int, float))
        ]
        if readings:
            depths[float(delay_us)] = sum(readings) / len(readings)
    if not depths:
        return None
    return max(depths, key=lambda coordinate: (depths[coordinate], -abs(coordinate)))
...
def sweep_verdict(
    ...
    * ``delay_resolved_robust`` — the selected coordinate nulls at or past
      :data:`ROBUST_NULL_DEPTH_DB` and every pose agrees on it.
    * ``delay_resolved_weak`` — a delay was selected, but either the null is
      shallower than the robustness bar or the poses disagree about where it
      sits. Prescribe it only with that stated.
    * ``axis_or_lobing_limited`` — no coordinate anywhere in the bounded grid
      reached :data:`USABLE_NULL_DEPTH_DB`. The residual at Fc is not a delay
      residual; prescribing a delay here would chase geometry with timing.
    * ``evidence_incomplete`` — the shared selector refused the evidence
      (missing or unrepeatable captures). Its own ``reason`` is carried through
    ...
    pose_best = {str(pose): _pose_best(rows_by_delay, pose) for pose in poses_deg}
    measured_poses = [value for value in pose_best.values() if value is not None]
    # A pose that lands anywhere on the plateau agrees: inside the measured
    # repeat spread the coordinates are one answer, not several.
    poses_agree = bool(measured_poses) and all(
        any(abs(value - coordinate) <= 1e-6 for coordinate in plateau)
        for value in measured_poses
    )
...
async def run_delay_sweep(
    ...
    try:
        ...
        for coordinate in coarse:
            await _measure_coordinate(plan, seams, live_graph, coordinate, rows_by_delay)
        axis = plan.poses_deg[0]
        schedule = BoundedNullWalkSchedule.from_coarse_evidence(
            spec,
            {value: rows for value, rows in
             rows_at_pose(rows_by_delay, axis).items() if value in coarse},
        )
    ...
    # The design axis is the first pose; it is the one the selection is made on.
    selection = select_scheduled_delay(
        spec, schedule, rows_at_pose(rows_by_delay, plan.poses_deg[0])
    )
...
async def _measure_coordinate(
    ...
    channels: tuple[int, ...] = ()
    if candidate.delay_target is not None:
        channels = tuple(plan.role_channels.get(candidate.delay_target, ()))
        if not channels:
            raise DelaySweepRefused(
                REFUSE_BRANCH,
                f"no physical channels declared for delay target "
                f"{candidate.delay_target!r}",
            )
    ...
    for pose_deg in plan.poses_deg:
        for _ in range(plan.repeats):
            ...
            row = capture_row(
                acoustic, pose_deg=pose_deg, relative_delay_us=coordinate
            )
            rows.append(row)
            plan.trail.append(
                {
