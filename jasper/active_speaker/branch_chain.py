...
Pure computation: where the driver radiates (:func:`radiating_band_hz`, the fit's band
bound) and how much the branch puts above unity
(:func:`branch_chain_peak_db`/:func:`branch_headroom_db`, what the emitter must absorb
...
def crossover_response_db(
    ...
    """The magnitude, in dB, that ``sections`` apply across ``freqs_hz``. LR order N is two
    cascaded Butterworth passes of N/2, built as the DIGITAL sections CamillaDSP
    realizes. The analog closed form under-reads a branch's true output by up to 1.02 dB
    (LR4) / 1.58 dB (LR2) at a 10 kHz crossover. Never positive: a Butterworth pass is
    monotonic and never exceeds unity, so a cut-only chain is known to sit at or below
...
def _butterworth_qs(order: int) -> list[float]:
    """The pole Qs of a Butterworth filter of ``order``, one per biquad section. An odd order's
    leftover real pole is :func:`_first_order_response`'s, not in this list.
    """
    return [
...
def radiating_band_hz(
    ...
    *,
    ...
    """The span where this branch is within ``edge_attenuation_db`` of full output -- the band
    a linearization fit is allowed to claim (#1809). Solved in closed form from the
    ANALOG LR prototype, not the digital filter :func:`crossover_response_db` builds:
    deliberately places the edge slightly INSIDE the real filter's 3 dB point (measured
    at LR4: -2.98 dB at 2 kHz, -2.43 at 10 kHz), so the fit may add level over slightly
    less of the spectrum, never more. ``(0.0, inf)`` for no sections. A mid squeezed
    between two crossovers closer than their own edges returns an EMPTY (``lo > hi``)
    band. A non-positive ``edge_attenuation_db`` raises.
    """
...
def beaming_onset_hz(radiating_diameter_mm: float, *, ka: float = BEAMING_KA) -> float:
    ...
    deploy/assets/sound-profile/js/main.js). Non-positive input raises.
    """
...
def branch_chain_peak(
    ...
    *,
    ...
    """:func:`branch_chain_peak_db`'s answer AND the frequency it occurs at. WHERE a chain
    peaks says whether it is the requested boost or an unpredicted cascade extremum
    (#2758's peak sits at 21.5 kHz). ``nan`` on the cut-only short-circuit, never 0.0
    (which would read as DC).
    """
...
def branch_headroom_db(
    ...
    *,
    ...
    :func:`branch_chain_peak_db`. The number the household is told, the emitter
    attenuates by, and the runtime contract proves.
    """
