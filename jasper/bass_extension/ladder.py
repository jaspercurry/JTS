...
clean ceiling, sustain result, anchor derivation) as pure functions over
Wave 1 numerics and the ``MarginPolicy`` thresholds.  Opens no device, socket,
CamillaDSP connection, subprocess, or coordinator; persists nothing; reads no
env var.  ``review -> accepted`` is Wave 3 and intentionally absent —
``accepted`` is not a reachable state.  Neither the injected
:func:`synthetic_dry_run` nor :func:`synthetic_limiter_intake` derives or
publishes any ``limiter_threshold_dbfs``.

Deferred (open question, not implemented here): plan §7.2's tracking-error-vs-
``predicted_response`` gate on ``verify_deepest -> ladder`` has no traced
pass/fail threshold in ``MarginPolicy`` or any prior wave doc. Per-rung
tracking error is still computed and retained as evidence
(``RungRecord.tracking_rms_db`` / ``tracking_max_db``); it just does not gate
anything yet. See ``wave-4-commissioning-backend.md`` "Open questions".
"""
...
# 20 Hz matches adapters.base.COMMISSION_FLOOR_HZ -- nothing is ever
# commissioned below that floor, so the THD extension band never needs to
# reach lower (plan §7.5 only names the upper edge: "below 80 Hz").
THD_EXTENSION_LO_HZ = 20.0
...
@dataclass(frozen=True)
class CeilingResult:
    """The ladder's clean-ceiling verdict (plan §7.4/§7.5).

    ``listening_level`` is a proven ceiling only when it is not ``None``. A
    ladder that never hit a stop-condition (nothing "stops tracking cleanly",
    per plan §7.4) sets ``refusal=LADDER_INCOMPLETE`` and leaves
    ``listening_level=None`` -- there is no rung to anchor "one below" from,
    so nothing may be published as a measured anchor.
    """
...
def transition(session: LadderSession, event: LadderEvent) -> LadderSession:
    ...
    if event.kind == _ABORT:
        if session.state in _TERMINAL:
            return session
        reason = event.reason or "operator_stop"
        return replace(session, state=LadderState.ABORTED, disposition=reason)
    try:
    ...
    if dest not in _FORWARD[session.state]:
        raise LadderTransitionError(
            f"transition {session.state.value} -> {dest.value} is not allowed"
        )
...
def validate_commissioning_manifest(raw: Mapping[str, Any]) -> CommissioningManifest:
    ...
    known_keys = {"margin", "enclosure_kind", "cabinet", "n_targets", "mic_calibration_id"}
    ...
    return CommissioningManifest(
        margin, adapter.adapter_id, enclosure_kind, cabinet, n_targets, mic
    )
...
def _rung_thd_max(m: RungMeasurement) -> float:
    """Max THD ratio in the extension band, over SNR-valid grid points.

    Band-edge points ``thd_curve`` masks to NaN (low fundamental SNR) are
    skipped rather than treated as a failure.
    """
    harmonics = {
        int(order): (np.asarray(freqs, dtype=np.float64), np.asarray(db, dtype=np.float64))
        for order, (freqs, db) in m.harmonics.items()
    }
    noise_floor = None
    if m.noise_floor is not None:
        noise_freqs, noise_db = m.noise_floor
        noise_floor = (
            np.asarray(noise_freqs, dtype=np.float64),
            np.asarray(noise_db, dtype=np.float64),
        )
    _, ratio = thd_curve(
        np.asarray(m.fund_freqs, dtype=np.float64), np.asarray(m.fund_db, dtype=np.float64),
        harmonics, band=(THD_EXTENSION_LO_HZ, THD_EXTENSION_HI_HZ), noise_floor=noise_floor,
    )
...
def mic_moved(
    prev_freqs: Sequence[float], prev_db: Sequence[float],
    cur_freqs: Sequence[float], cur_db: Sequence[float],
    commanded_delta_db: float,
) -> bool:
...
def evaluate_rung(
    compression_row: Sequence[float], thd_max: float,
    *,
    mic_moved_flag: bool, capture_clipped: bool,
    repeat_spread_db: float, snr_ok: bool, digital_reached: bool,
    margin: MarginPolicy, decision_band_hz: tuple[float, float],
    band_edges: tuple[tuple[float, float], ...] = THIRD_OCTAVE_BASS_BANDS_HZ,
) -> RungVerdict:
...
def run_ladder(
    measurements: Sequence[RungMeasurement], margin: MarginPolicy
) -> tuple[tuple[RungRecord, ...], CeilingResult]:
    """Evaluate a rung series; stop on the first failure, ceiling = previous rung."""
    if not measurements:
    ...
    for i, m in enumerate(measurements):
        ...
        verdict = evaluate_rung(
            compression[i], thd_max,
            mic_moved_flag=moved, capture_clipped=m.capture_clipped,
            repeat_spread_db=m.repeat_spread_db, snr_ok=m.snr_ok,
            digital_reached=m.digital_reached, margin=margin,
            decision_band_hz=m.decision_band_hz,
        )
        records.append(
            RungRecord(
                m.rung_ordinal, float(m.commanded_main_volume_db), m.listening_level,
                m.capture_id, levels[i], compression[i], thd_max, tr_rms, tr_max, verdict,
            )
        )
        if not verdict.passed:
            prev_record = records[i - 1] if i > 0 else None
            ceiling = CeilingResult(
                prev_record.listening_level if prev_record else None,
                verdict.limited_by, verdict.refusal,
                prev_record.rung_ordinal if prev_record else None,
            )
            break
        prev = m
    if ceiling is None:
        last = records[-1]
        # Nothing failed: no rung "stops tracking cleanly" (plan §7.4), so no
        # rung is a proven ceiling. refusal set -> no promoted listening_level
        # (see CeilingResult docstring) -- an incomplete ladder is a refusal,
        # not a win, and must not be read as clean headroom to the top rung.
        ceiling = CeilingResult(
            None, None, BassExtensionRefusal.LADDER_INCOMPLETE, last.rung_ordinal
        )
    return tuple(records), ceiling


def sustain_result(
    pre_band_levels: Sequence[float],
    post_band_levels: Sequence[float],
    refit_fc_pre_hz: float,
    refit_fc_post_hz: float,
    margin: MarginPolicy,
) -> SustainResult:
    ...
    fc_pre = float(refit_fc_pre_hz)
    fc_shift_pct = abs(float(refit_fc_post_hz) - fc_pre) / fc_pre * 100.0
    limited_by = None
    ...
    verdict = "lowered" if limited_by else "passed"
    return SustainResult(verdict, sag, fc_shift_pct, margin.sustain_duration_s, limited_by)


def apply_sustain(
    rungs: Sequence[RungRecord], ceiling: CeilingResult, sustain: SustainResult
) -> CeilingResult:
    """Return the final ceiling: unchanged on pass, one clean rung lower on fail.

    A ``ceiling`` that already carries a ``refusal`` (e.g. ``LADDER_INCOMPLETE``
    — nothing stopped the ladder cleanly) was never a proven ceiling, so there
    is nothing to lower it from: it is returned unchanged, and the refusal is
    never erased by manufacturing a fresh, unrefused result.
    """
    nothing_to_lower = sustain.verdict == "passed" or ceiling.refusal is not None
    if nothing_to_lower or ceiling.ceiling_rung_ordinal is None:
        return ceiling
    lower = [
        r for r in rungs if r.rung_ordinal < ceiling.ceiling_rung_ordinal and r.verdict.passed
    ]
    if not lower:
...
def synthetic_dry_run(inputs: DryRunInputs) -> LadderSession:
    """Walk ``idle -> review`` with injected fakes; opens no device or I/O.

    Reaches at most ``review`` — ``review -> accepted`` is Wave 3.
    """
    manifest = validate_commissioning_manifest(inputs.manifest)
    margin = MARGINS[manifest.margin]
    session = LadderSession(
        session_id=inputs.session_id, margin=manifest.margin, adapter_id=manifest.adapter_id,
        cabinet=manifest.cabinet, n_targets=manifest.n_targets,
        mic_calibration_id=manifest.mic_calibration_id,
    )

    pre = preflight(inputs.preconditions)
    if not pre.ok:
        aborted = transition(session, LadderEvent(_ABORT, reason="preflight_blocked"))
        return replace(aborted, refusals=pre.blocking)

    session = transition(
        session, LadderEvent("characterize", updates={"captures": inputs.capture_records})
    )
    fit = inputs.adapter.fit_plant(inputs.capture_curves, manifest.cabinet)
    if isinstance(fit, FitRefusal):
        aborted = transition(session, LadderEvent(_ABORT, reason="fit_refused"))
        return replace(aborted, refusals=(fit.refusal,))
    session = transition(session, LadderEvent("fit", updates={"plant_fit": fit}))

    family = inputs.adapter.generate_family(fit, margin=margin, n_targets=manifest.n_targets)
    ...
    session = transition(
        session, LadderEvent("ladder", updates={"rungs": records, "ceiling": base_ceiling})
    )
    ...
    session = transition(
        session,
        LadderEvent("sustain_test", updates={"sustain": sustain, "ceiling": final_ceiling}),
    )
    if final_ceiling.listening_level is None:
        aborted = transition(session, LadderEvent(_ABORT, reason="ladder_incomplete"))
        refusal = final_ceiling.refusal or BassExtensionRefusal.LADDER_INCOMPLETE
        return replace(aborted, refusals=(refusal,))

    measured = (
        AnchorPoint(family[0].target_id, final_ceiling.listening_level, "measured"),
        *inputs.spot_anchors,
    )
    anchors = interpolate_anchors(family, measured, margin)
