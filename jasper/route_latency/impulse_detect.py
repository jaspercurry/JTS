...
identical algorithm) and this package's egress (mic) detector. Keeping the
detection logic in exactly one Python place — rather than reimplementing it
per caller — means a threshold/hysteresis/refractory tuning change only has
one home, and a synthetic-signal test proves both legs would agree on the
same click.

Algorithm (peak + hysteresis + refractory), matching the pinned Rust tap
contract bit-for-bit in spirit:
...
in-memory ``int16`` sample buffers so it is trivially unit-testable and
shared by both the mic-egress reader and the click-track's own
self-verification tooling.
"""
...
class StreamingDetector:
    ...
    Mirrors the Rust tap's per-period detector so both ends of a
    measurement agree on what counts as "the click." Call :meth:`feed` once
    per arriving chunk of ``int16`` samples (a UDP packet, an ALSA period, a
    whole click-track buffer for self-test); it returns zero or more
    :class:`Detection` for that chunk.
    """
