...
| A1 | Safe CamillaDSP templates | Started 2026-06-01 as a no-apply startup-template emitter with muted/protected 2-way/3-way output chains, explicit mixer output labels, and CLI `camilladsp --check` when available. Still needs rollback statefile handling and hardware loading gates. |
| A2 | Engineering preset interop | Started 2026-06-01 with a data-only DE250 + E150HE-44 worked-example preset. Still needs REW/VituixCAD artifact references, expected envelopes, safe sweep ranges, trims, limiters, and BSC. |
| A3 | Channel/path safety | Started 2026-06-01 with `jasper.active_speaker.path_safety` and `jasper-active-speaker path-audit`: deterministic requirements/evidence evaluation only. Expanded 2026-06-02 with read-only `jasper-active-speaker environment-probe` for ALSA playback-device discovery, current/provided CamillaDSP config classification, volume-limit/channel inspection, optional `camilladsp --check`, and an explicit `safe_playback` block that keeps tone playback unauthorized. The early standalone `tone_plan` channel-test planner was superseded and removed; current per-driver product tests use `test_signal_plan` through the protected commission ramp, with `safe_playback` retaining bounded arm/stop state. Operator evidence alone still cannot authorize active config loading. Systemd/source routing, hardware-probe-backed path evidence, physical output verification, and the Stop-controlled ramp remain required before drivers are audible. |
