...
"""Audio runtime plan assembly, route policy, observations, and writer actions."""
...
from jasper.audio_runtime_settings import (
    ...
    MAX_LOW_LATENCY_CORRECTION_GROUP_DELAY_FRAMES,
    ROUTE_CONFIG_HASH_SCHEMA_VERSION,
    ROUTE_CORRECTED_48K,
    ROUTE_USB_LOW_LATENCY_48K,
    RouteMode,
    UAC2_LOW_LATENCY_EXPECTED_ATTRS,
    USB_LOW_LATENCY_SOURCE_ID,
    VALID_AUDIO_ROUTE_PROFILES,
    VALID_ROUTE_MODES,
    AUDIO_RUNTIME_OVERRIDE_KEYS,
    DEFAULT_FANIN_INPUT_BUFFER_FRAMES,
    DEFAULT_FANIN_USB_DIRECT_PERIOD_FRAMES,
    DEFAULT_OUTPUTD_DAC_BUFFER_FRAMES,
    DEFAULT_OUTPUTD_PERIOD_FRAMES,
    DEFAULT_USB_LOW_LATENCY_RESAMPLER_CUSHION_FRAMES,
    DEFAULT_USB_LOW_LATENCY_RESAMPLER_MAX_ADJUST_PPM,
    DEFAULT_USB_LOW_LATENCY_RESAMPLER_RING_FRAMES,
    ...
    FANIN_USB_DIRECT_DEVICE,
    FANIN_USB_DIRECT_MIN_BUFFER_FRAMES,
    FANIN_USB_DIRECT_MIN_BUFFER_PERIODS,
    FANIN_USB_DIRECT_PERIOD_KEY,
    MAX_FANIN_USB_DIRECT_PERIOD_FRAMES,
    MIN_FANIN_USB_DIRECT_PERIOD_FRAMES,
...
def resolve_audio_route_profile(
    ...
    warnings: tuple[str, ...] = ()
    if route_id not in VALID_AUDIO_ROUTE_PROFILES:
        warnings = (
...
def outputd_config_for_settings(
    ...
    return {setting.key: setting.value for setting in settings if setting.key in keys}
...
def route_config_hash_for_plan(
    *,
    ...
    payload = {
        ...
        "route": route.to_dict(),
        "route_env_actions": [
            action.to_dict() for action in route_owned_env_actions(route)
        ],
...
def _route_policy_errors(
    *,
    ...
    if dac_content_lane_marker_armed(outputd_env):
        ...
        raw_bridge = (
            str(outputd_env.get(OUTPUTD_CONTENT_BRIDGE_ENV_VAR) or "").strip().lower()
        )
        errors.append(
            (
...
def build_audio_runtime_plan(
    *,
    ...
    camilla_chunksize_setting = resolve_profile_floor_int(
    ...
    camilla_target_setting = resolve_profile_floor_int(
    ...
    return AudioRuntimePlan(
        ...
        route_mode=route_mode if route_mode in VALID_ROUTE_MODES else "unknown",
        settings=tuple(settings),
    ...
    *paths: str | Path,
    ...
    for statefile_path in paths:
        try:
            ...
            errors.append(
                f"statefile {statefile_path}: {e.strerror or type(e).__name__}"
            )
            continue
...
def _fir_metadata_paths_for_config(text: str, *, config_path: Path) -> tuple[Path, ...]:
    out: list[Path] = []
    for filename in _WAV_FILENAME_RE.findall(text):
        wav_path = Path(filename.strip())
...
def outputd_latency_floor_actions(
    *,
    ...
    for key in OUTPUTD_LATENCY_KEYS:
        override_value, _ = positive_int(raw(override_values, key))
