...
use jasper_ring::{Geometry, RingWriter, SAMPLE_FORMAT_S32LE};
...
use ring_output::{
    ring_open_error, write_ring_period, PeriodPacer, RingCounters, RingObservability, RingOutput,
    RingStallTracker,
};
...
mod tests {
    ...
    // Lane gates, drain and PCM error classification. The pure mix math has
    // its tests in dsp.rs, the ALSA open helpers theirs in pcm_open.rs, and
    // the ring publish path, stall and pacing theirs in ring_output.rs.
    //
    ...
    pub(super) fn ring_geometry(n_slots: u32) -> Geometry {
        Geometry {
            rate: 48_000,
            channels: CHANNELS,
            sample_format: SAMPLE_FORMAT_S32LE,
    ...
    pub(super) fn tmp_ring_output(n_slots: u32, tag: &str) -> (RingOutput, String) {
    ...
    pub(super) fn payload_of(sum: &[i64]) -> Vec<u8> {
    ...
    pub(super) fn samples_of(slot: &[u8]) -> Vec<i32> {
        slot.chunks_exact(BYTES_PER_SAMPLE)
            .map(|c| i32::from_le_bytes(c.try_into().unwrap()))
            .collect()
    }

    pub(super) fn cleanup_ring(path: &str) {
