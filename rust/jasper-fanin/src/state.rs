...
use crate::mixer::{CouplingObservability, Mixer, TrimControl, OUTPUT_DELAY_UNAVAILABLE};
...
impl StateServer {
    ...
    /// capture `PCM` (it lives on the mixer work thread), this sets each target
    /// lane's `pending` flag with a `Release` store and then briefly polls the
    /// lane's `trimmed_frames` counter for the work loop to consume the flag and
    /// publish the delta — reporting the frames ACTUALLY dropped, not just "the
    /// request was queued." Mirrors the SELECT/AUTO/NONE split (control sets a
    /// shared atomic; the work loop does the I/O).
    ///
    ...
    /// operator wants the one number back on the socket immediately.
    fn trim_command(&self, label: Option<&str>) -> String {
        ...
        // 0 frames (lane already shallow) still clears its pending flag, so we
        // wait on the flags clearing, not on the counter moving — a legitimate
        // 0-frame trim must not spin out the whole timeout.
    ...
    pub fn snapshot_json(&self) -> String {
        ...
        for (i, input) in self.inputs.iter().enumerate() {
            ...
            // is the cumulative total dropped; `trim_pending` shows an armed but
            // not-yet-serviced request. All 0/false on a lane never trimmed, so
            // the shape is stable for the common case. Always present (unlike the
            // optional resampler block) — a flat, greppable pair like the
            // catch-up counters above.
            buf.push_str(r#""trim":{"#);
