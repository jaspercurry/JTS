...
pub use jasper_host_clock::{
    ctl_card_from_capture, ppm_to_ctl_value, Action, HostClock, HostClockConfig, Ladder, Obs,
    PitchCtl, ProbeResult, ANTI_WINDUP_THRESHOLD_FRAMES, L1_RELEASE_PPM, L1_SUSTAIN_TICKS,
    L1_WARN_PPM, L2_SLOPE_FLOOR_PPM, L2_SUSTAIN_TICKS, MAX_BIAS_PPM, PITCH_NEUTRAL,
    TICK_INTERVAL_MS, WRITE_EPSILON_PPM, WRITE_MIN_INTERVAL_MS,
};
...
/// process environment.
pub fn from_env<F>(getenv: F, period_frames: u32) -> Result<HostClockConfig, String>
where
...
pub fn disabled_config(period_frames: u32) -> HostClockConfig {
    HostClockConfig::disabled(period_frames, LOG_PREFIX)
}
...
mod tests {
    ...
    fn config_disabled_by_default_and_inert() {
        let cfg = from_env(|_| None, 256).unwrap();
        assert!(!cfg.enabled);
    ...
    fn config_enabled_only_on_literal_enabled() {
        ...
        assert!(from_env(get, 256).unwrap().enabled);
        ...
        assert!(!from_env(get_other, 256).unwrap().enabled);
    }
    ...
    fn config_rejects_out_of_range_tunables() {
        ...
        .is_err());
        assert!(from_env(with("JASPER_USBSINK_HOST_CLOCK_PROBE_PPM", "50"), 256).is_err());
        // Below/at the ~163 ppm Windows deadband is rejected (S4): a probe
        // there would falsely fail every session on a compliant host.
        assert!(
            from_env(with("JASPER_USBSINK_HOST_CLOCK_PROBE_PPM", "100"), 256).is_err(),
            "PROBE_PPM=100 (<= ~163 ppm deadband) must be rejected"
        );
        assert!(from_env(with("JASPER_USBSINK_HOST_CLOCK_PROBE_PPM", "199"), 256).is_err());
        // The floor itself (200, just above the deadband) is accepted.
        assert!(
            from_env(with("JASPER_USBSINK_HOST_CLOCK_PROBE_PPM", "200"), 256).is_ok(),
            "PROBE_PPM=200 (above the deadband) must be accepted"
        );
        assert!(from_env(with("JASPER_USBSINK_HOST_CLOCK_PROBE_PPM", "1200"), 256).is_err());
        assert!(from_env(with("JASPER_USBSINK_HOST_CLOCK_PROBE_SECONDS", "3"), 256).is_err());
        assert!(from_env(with("JASPER_USBSINK_HOST_CLOCK_PROBE_SECONDS", "20"), 256).is_err());
    }
