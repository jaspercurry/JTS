...
impl OutputdState {
    ...
    pub(super) fn reference_outputs_json(
        &self,
        buf: &mut String,
        sample_rate: u64,
        uptime_ms: u64,
        dac_delay_frames: Option<u64>,
    ) {
        buf.push_str(r#""reference_outputs":{"#);
        push_kv_str(buf, "speaker_reference_source", "outputd_final_electrical");
        buf.push(',');
        push_kv_str(
            buf,
            "chip_ref_transform",
        ...
        buf.push(',');
        push_kv_bool(buf, "speaker_reference_is_fallback", false);
        buf.push(',');
        push_kv_bool(
            buf,
        ...
        buf.push(',');
        push_kv_u64(buf, "speaker_reference_sample_rate", sample_rate);
        buf.push(',');
        push_kv_u64(
            buf,
        ...
        buf.push(',');
        push_kv_str_opt(buf, "chip_ref_pcm", self.chip_ref_pcm.as_deref());
        buf.push(',');
        push_kv_u64(
            buf,
            "chip_ref_sample_rate",
        ...
        buf.push(',');
        let chip_ref_sample_rate = self.chip_ref_writer_json(buf, uptime_ms);
        push_kv_str_opt(buf, "udp_target", self.reference_udp_target.as_deref());
        buf.push(',');
        push_kv_bool(
            buf,
            "udp_active",
        ...
        push_kv_f64_opt(buf, "chip_ref_sro_ppm", sro_ppm, 3);
        buf.push(',');
        push_kv_str(buf, "sro_estimator_status", sro_status);
        buf.push(',');
        push_kv_str(buf, "verdict", verdict);
        buf.push(',');
        push_kv_str(buf, "verdict_reason", &verdict_reason);
        buf.push(',');
        ...
        push_kv_bool(buf, "observe", self.chip_ref_observe);
        buf.push(',');
        buf.push_str(r#""latency":{"#);
        push_kv_f64_opt(buf, "dac_presentation_ms", dac_presentation_ms, 3);
        buf.push(',');
        push_kv_f64_opt(buf, "playback_queue_ms", playback_queue_ms, 3);
        buf.push(',');
        push_kv_f64_opt(buf, "chip_ref_queue_ms", chip_ref_queue_ms, 3);
        buf.push('}');
        buf.push('}');
        buf.push('}');
        buf.push(',');
