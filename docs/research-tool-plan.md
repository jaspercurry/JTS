...
| Failure speech | Phase 3 plays the provider-agnostic `research_failed` cue for runtime failures and restart-interrupted jobs, rate-limits proactive failed-job audio to once per hour, and adds a not-configured prompt redirect to `/assistant/voice/`. | No-silent-failure: an out-of-band job has no wake event to hang a reactive cue on, so failures must speak. The cooldown prevents a burst of failures from nagging the household. |
