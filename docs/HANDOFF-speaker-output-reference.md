...
`jasper-outputd`, Camilla output wiring, or any future output-owner
daemon. For source-specific lane
work, start with
...
active-endpoint safety exception. The legacy `pcm.jasper_out` dmix and
its `v1.yml` config were retired (issue #2240); rollback to the
...
- On a passive bonded member, the grouping reconciler routes TTS to outputd and
  ...
  to 0 — the structural-zero fact belongs to the post-DSP consumer). Voice
  embeds the turn-start context atomically in `PREPARE_ASSISTANT`; outputd
  clears any prior context and keeps speech silent if that context is absent or
  rejected, so a restart, read failure, or legacy four-field prepare cannot
  inherit an old unmuted state. Outputd
  ...
  cannot drift. The fail-closed rule stands: outputd emits assistant audio only
  with a valid turn-start context and applies post-DSP compensation only when its
  engine is explicitly `MixStage::PostDsp`; a legacy socket-only grouping
  override stays ambiguous and silent.
...
- `deploy/camilladsp/outputd-cutover.yml` sends Camilla playback to
  ...
  first-boot bytes equal regenerated bytes). The pre-outputd rollback
...
the prior `jasper_out` topology (retired from the tree as of issue
#2240) is explicit: run
...
- DAC output: `outputd_dac`, a direct hardware alias for the selected
  ...
  HiFi AMP Pro uses the enumerated `sndrpimerusamp` card. Two profiles now
  declare a non-S16 final edge (`DacProfile.final_edge_format = S32_LE`):
  ...
  this program does not flip it on inference from a shared DAC-chip family
  alone. The two boards do NOT share an overlay or a driver: the Studio has
  its own `hifiberry-studio-dac8x` overlay and its own machine driver, added
  to raspberrypi/linux 2026-01-15, and the base profile's regexes were
  narrowed to the one card name its driver emits so Studio silicon can no
  longer be classified `hifiberry_dac8x` and inherit `S32_LE` (#2250).
  Two residuals are documented rather than closed: a Studio board configured
  with the base overlay presents the base card name and is genuinely
  indistinguishable, and on rpi-6.18.y and later the whole Studio family
  shares one card name, so a Studio DAC8x parks as `unknown` (#2258). outputd
