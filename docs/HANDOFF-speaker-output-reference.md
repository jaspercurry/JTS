...
- `deploy/camilladsp/outputd-cutover.yml` sends Camilla playback to
  ...
  first-boot bytes equal regenerated bytes); `deploy/camilladsp/v1.yml` is
  retained as the pre-outputd rollback config that writes to `jasper_out`
  and stays `S16_LE` — that dmix is pinned narrow and has no outputd spine
  behind it.
