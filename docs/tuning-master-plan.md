...
- **`verify` program:** the reverse-null polarity check (flip the tweeter:
  ...
  host that executes it: it walks the grid against the applied crossover
  with one branch inverted, and grades the result. `jasper-delay-sweep`
  is the operator door onto that walk's plan and its grading.
