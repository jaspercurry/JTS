...
> **Status: design-only.** This is a research + design proposal,
> **not yet implemented** — no `jasper/dlna/`, no
> `gmediarender.service`, no DLNA substream lane exists in the
> codebase today. The audio-path integration described here is
> written against the **current** fan-in / `jasper-outputd`
> topology (per-source snd-aloop lanes summed by the Rust
> `jasper-fanin` daemon, then CamillaDSP, then `jasper-outputd`
> owns the DAC). Before implementing, read
> [`docs/audio-paths.md`](audio-paths.md) "Adding a new music
> source" and [`docs/HANDOFF-fan-in-daemon.md`](HANDOFF-fan-in-daemon.md)
> — those are the operational source of truth for the topology;
> this doc adds the DLNA-specific shape on top. The Cast-vs-DLNA
> research, renderer analysis (gmrender vs upmpdcli), and decision
> records below are the durable content.
...
DLNA/UPnP audio rendering becomes an additional network-only
music source alongside AirPlay, Spotify Connect, Bluetooth A2DP,
and USB Audio Input. The speaker appears on the LAN as a UPnP
Media Renderer; any DLNA controller app can discover it and
stream audio to it. (No roadmap ordinal is claimed here — DLNA
sits in the post-USB-sink new-sources cluster alongside Apple
Music; see PLAN.md.)
...
- Audio output to a **dedicated private snd-aloop fan-in lane**
  (a new `*_substream` alias) — the same per-source lane pattern
  AirPlay, Spotify Connect, Bluetooth, and USB Audio use. The
  Rust `jasper-fanin` daemon sums that lane into substream 7;
  CamillaDSP reads the sum; `jasper-outputd` owns the DAC.
- Latest-source-wins arbitration via `jasper-mux`, preempted
  through the sidecar's localhost `/preempt` endpoint (UPnP
  `Pause`→disarm, falling back to `Stop`)
...
The DLNA source reuses the existing per-source fan-in lane
...
gmrender's `alsasink` writes to a **new dedicated DLNA substream
alias** (e.g. `pcm.dlna_substream`, a `plug:` wrapper over a
private `hw:Loopback,0,N` lane). It does **not** write to a
shared renderer dmix — the earlier shared-renderer-dmix topology
was retired in favor of per-source snd-aloop lanes (jasper-doctor
now flags that retired dmix alias as stale, and
`tests/test_fanin_wiring.py` asserts it no longer exists).
`jasper-fanin` sums every active source lane (including this one)
into substream 7; CamillaDSP reads that summed stream;
`jasper-outputd` is the final-output owner that writes the DAC.
...
gmediarender (UPnP Media Renderer, C binary)
    │
    │ GStreamer → alsasink device=dlna_substream
    ▼
pcm.dlna_substream (plug → hw:Loopback,0,N)   ← new private lane
    │
    ▼ (snd-aloop loop)
hw:Loopback,1,N → jasper-fanin (sums active lanes) → hw:Loopback,0,7
    │
    ▼ (snd-aloop loop)
pcm.jasper_capture (dsnoop on hw:Loopback,1,7)
    │
    ▼
jasper-camilla (main_volume + filters) → outputd_content_playback
    │
...
gmediarender \
    ...
    --gstout-audiodevice dlna_substream \
    --logfile /dev/stderr
```
...
| `--gstout-audiodevice` | ALSA device for the sink — the new private `dlna_substream` fan-in lane (NOT a shared dmix) |
...
the user taps pause. This is a non-issue for JTS because each
source writes to its **own private snd-aloop substream lane**
(`dlna_substream` → a dedicated `hw:Loopback,0,N`), not a shared
device. gmrender holding its private lane open across a pause
doesn't block any other renderer — every other source has its
own lane, and `jasper-fanin` simply mixes whatever each lane
currently produces (silence while paused). The mux preempt path
(§3.2) still sends Pause→disarm so a paused-then-preempted DLNA
session doesn't auto-resume audio into the summed output.
...
│ controller   │           │  (C binary, gmrender)     │           pcm.dlna_
│ (phone app)  │ ◄──────── │                           │           substream
│              │   UPnP    │  port: dynamic (libupnp)  │        (→ jasper-fanin)
└──────────────┘  events   └──────────────────────────┘
                                          ▲
                                          │ UPnP Pause
                              ┌───────────┴───────────┐
...
2. **Session**: Phone sends `SetAVTransportURI` + `Play` SOAP actions. gmrender fetches audio from the URL in the action, decodes via GStreamer, outputs to `alsasink device=dlna_substream` (its private fan-in lane). `jasper-fanin` sums that lane (when the DLNA lane is the selected/active source) into substream 7 for CamillaDSP.
...
### 3.3 Source declaration + volume bucket (`jasper/music_sources.py`)

The `Source` enum and the per-source volume policy are **no
longer hardcoded tuples** in `volume_coordinator.py`. They are
declared once in `jasper/music_sources.py`:

- Add a `Source.DLNA = "dlna"` enum member.
- Add one `MusicSourceSpec` with `volume_mode=VolumeMode.CAMILLA_MASTER`
  (same bucket as AirPlay and USB Audio Input), a stable
  `fanin_label` (e.g. `"dlna"`, matching the fan-in lane label),
  `renderer_active_key="dlnaactive"`, `wizard_key="dlna"`, and a
  display name.

`VolumeCoordinator._camilla_carries_level()` already reads
`volume_mode(source) == VolumeMode.CAMILLA_MASTER`, so declaring
the spec is the entire volume-bucket wiring — there is no tuple to
edit in `volume_coordinator.py` and no `_set_dlna()` dispatcher to
add (camilla carries the level, same as AirPlay). The `Source` enum
member and the spec also feed mux's allow-list, manual source
selection, and the doctor topology checks, so this one declaration
is load-bearing across several callers.

### 3.3.1 Source-handoff safety (mux ↔ VolumeCoordinator)

This is a **required** integration the current "adding a new music
source" checklist calls out, and it is the part most easily missed
for a camilla-master source. Before mux moves the fan-in gate to the
DLNA lane it must call
`VolumeCoordinator.prepare_source_handoff(...)`, and after the gate
moves, `VolumeCoordinator.finalize_source_handoff(...)`. Mux already
does this for every transition (see
[`docs/audio-paths.md`](audio-paths.md) "Manual source selection"
and `jasper/mux.py`); DLNA simply participates.

Why it matters here: DLNA is camilla-as-master. Taking over from a
**push-mode** source (Spotify/Bluetooth, where CamillaDSP sits at
0 dB and the source slider carries `listening_level`) means
CamillaDSP's `main_volume` must be raised to carry the level for
DLNA. Doing that *before* the gate moves, via the handoff
transaction, is what prevents a loud source-switch transient —
exactly the Spotify (Camilla 0 dB) → AirPlay (Camilla-as-master)
guard documented in audio-paths.md, now applied to DLNA. Skipping
the handoff would let a camilla-master DLNA source blast a loud
transient when it takes over.
...
### 3.9 Fan-in lane registration (`deploy/alsa/asoundrc.jasper` + `rust/jasper-fanin/src/config.rs`)

The audio path is wired in two places, not a `--gstout-audiodevice`
that points at a shared dmix:

1. **ALSA alias.** Add one `pcm.dlna_substream` `plug:` wrapper in
...
2. **Fan-in input list.** Register the capture side and a stable
   ...
   `Config::from_env` — extend both the `input_pcms` default array
   (add `hw:Loopback,1,N`) and the `input_renderers` default array
   (add `"dlna"`), keeping them positionally aligned. A configured
   fan-in input is part of the production graph: if it can't be
   opened, fan-in fails loudly rather than silently dropping the
...
A room-correction sweep must pause every source that can emit
during the measurement window, or the sweep picks up music as if it
were the test tone. Add the DLNA renderer service to
`DEFAULT_RENDERERS_TO_PAUSE` in
...
DEFAULT_RENDERERS_TO_PAUSE: tuple[str, ...] = (
    ...
    "gmediarender.service",   # ← add: pause DLNA during a sweep
)
```

This is the checklist's "add the source to the correction pause
list if it can emit during a sweep" requirement. Pausing the
renderer service (not the sidecar) is correct: the sidecar is a
pure observer and emits no audio.
...
After=sound.target network-online.target jasper-fanin.service
Wants=network-online.target
Requires=jasper-fanin.service
...
**Key decisions:**
- `--gstout-audiodevice dlna_substream` — the private fan-in lane
  (§3.9), NOT a shared renderer dmix
- `After=`/`Requires=jasper-fanin.service` — the lane's summing
  consumer must be up first, matching how the other renderer units
  order after fan-in (per the audio-paths.md checklist)
...
### 5.6 Audio-path wiring is a source-tree change, not an apt step

The DLNA fan-in lane (§3.9) is **not** installed by an
`install.sh` package action. It lands through the normal deploy of
two source-controlled files:

- The `pcm.dlna_substream` alias is added to
  `deploy/alsa/asoundrc.jasper`; `install.sh` already renders that
  template to `/etc/asound.conf` via `jasper-render-asound-conf`
  on every deploy, so the new alias ships automatically once the
  template carries it. No new installer logic — just confirm the
  rendered `/etc/asound.conf` contains `dlna_substream` after
  deploy.
- The fan-in `input_pcms` / `input_renderers` defaults are compiled
  into the `jasper-fanin` Rust binary (`rust/jasper-fanin/src/config.rs`),
  which `install.sh` already `cargo build`s and installs. The DLNA
  lane is live after the binary is rebuilt; no env override is
  required (and the `JASPER_FANIN_INPUT_*` env vars are an optional
  override, not wired into the unit by default).

Because the substream allocation is full (§3.9), the *first*
implementation step is settling which lane DLNA reuses — that
decision changes both files above and the modprobe
`snd-aloop.conf` pin if the count ever needs to grow (it should
...
| ALSA device unavailable | gmrender can't open `dlna_substream` | The snd-aloop lane is always available (snd-aloop is loaded at boot); if somehow gone, systemd restart retries. If `jasper-fanin` is down, gmrender's lane fills but nothing is summed — `Requires=jasper-fanin.service` ties the renderer to its consumer |
...
| Path | Change |
|---|---|
| `jasper/music_sources.py` | Add `Source.DLNA` enum member + one `MusicSourceSpec` (`volume_mode=CAMILLA_MASTER`, `fanin_label="dlna"`, `renderer_active_key="dlnaactive"`, `wizard_key="dlna"`). This is the canonical source declaration — mux allow-list, volume bucket, and selection derive from it (§3.3) |
| `deploy/alsa/asoundrc.jasper` | Add `pcm.dlna_substream` `plug:` alias over a private `hw:Loopback,0,N` lane (§3.9) |
| `rust/jasper-fanin/src/config.rs` | Add the lane's capture side to `input_pcms` + `"dlna"` to `input_renderers` (positionally aligned) (§3.9) |
| `jasper/source_state.py` | Add `dlna_playing()` probe |
| `jasper/mux.py` | Preemption via sidecar `/preempt` POST (same shape as usbsink); participates in `prepare_source_handoff`/`finalize_source_handoff` (§3.3.1) |
| `jasper/correction/coordinator.py` | Add `gmediarender.service` to `DEFAULT_RENDERERS_TO_PAUSE` (§3.10) |
| `jasper/renderer.py` | Add `dlna_playing()` to `active_renderers()` (key `dlnaactive`) |
| `jasper/web/sources_setup.py` | Add `"dlna"` toggle |
| `jasper/control/server.py` | Add `renderers.dlna` to `/state` |
| `jasper/cli/doctor.py` | Add `check_dlna_renderer()`, `check_dlna_gstreamer()`; optionally assert the `dlna_substream` alias + fan-in lane registration |
| `deploy/install.sh` | Add `install_dlna_renderer()`, user creation, UUID seeding, unit installation (the lane itself ships via the rendered asoundrc + rebuilt fan-in binary — §5.6) |
...
| `docs/doc-map.toml` | Add `docs/HANDOFF-dlna.md` to the `audio-routing-and-renderers` docs list |
...
- **Settle the substream lane first** (§3.9) — which existing lane
  DLNA reuses, since allocation is full. This gates the asoundrc +
  fan-in edits.
- Install gmrender-resurrect (apt or source-build)
- Create system user `jasper-dlna`
- Add `pcm.dlna_substream` to `deploy/alsa/asoundrc.jasper`;
  register the lane in `rust/jasper-fanin/src/config.rs`
- Write `gmediarender.service` (after/Requires `jasper-fanin`) +
  `jasper-dlna.service`
- Implement `jasper/dlna/` sidecar (daemon, state_publisher,
  upnp discovery)
- Add `dlna_playing()` to `source_state.py`
- Declare `Source.DLNA` + `MusicSourceSpec` in `music_sources.py`
  (drives volume bucket + mux allow-list; §3.3)
- Wire mux preemption + `prepare/finalize_source_handoff` (§3.3.1)
- Add `gmediarender.service` to correction `DEFAULT_RENDERERS_TO_PAUSE`
  (§3.10)
...
**Acceptance criteria:**
- Audio plays through the fan-in → CamillaDSP → outputd chain
- Mux preempts DLNA when another source starts, with no loud
  transient on a push-mode → DLNA handoff
- A room-correction sweep pauses DLNA
...
| Output device is the shared `dlna_substream` fan-in lane (not a gmrender-specific path) | Free | MPD/upmpdcli writes the same ALSA PCM |
...
2. Point MPD's ALSA output at the `dlna_substream` fan-in lane
...
The current ALSA topology (per-source `*_substream` lanes →
`jasper-fanin` sums into `hw:Loopback,0,7` → `pcm.jasper_capture`
→ CamillaDSP → `jasper-outputd` → DAC) is compatible with a future
Snapcast insertion. DLNA is just one more fan-in input lane, so it
inherits whatever the summed-output stage becomes. When multi-room
arrives:

- Insert Snapcast at the **summed-output boundary** — feed
  `jasper-fanin`'s mixed stream (or `pcm.jasper_capture`) into a
  Snapcast input FIFO instead of (or before) CamillaDSP, rather
  than redefining any per-source lane.
- All sources (shairport-sync, librespot, bluealsa, usbsink,
  gmrender) already converge at that one summed stream, so none of
  the per-source `*_substream` lanes change.
- CamillaDSP moves to the snapclient side (one instance per
  physical speaker); `jasper-outputd` stays the per-node final
  output owner.
- "Camilla-as-master" volume model still holds per-node.

This is a non-breaking topology extension because the fan-in
summed stream is the single, stable insertion point — exactly the
property the per-source-lane design buys. **No Phase 1 changes
...
records re-read and left intact.)
