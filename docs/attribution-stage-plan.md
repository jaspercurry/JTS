...
> P0–P4 course-correction ladder that reshapes the *order*: rung P0 replaces
> WO-7's unmeasured repeatability term, P1 gates WO-5's frame-anchor question
> (Q-E), P2 gates WO-4's mechanism freeze (§8 item 3), P3 gates WO-3's delay
> adoption (Q-A), and P1/P3/P4 all precede WO-6. **The ladder is pending owner
> ratification**, so those items are *superseded-pending*: do not execute them
> as written, and do not treat the ladder as decided either. The reconciliation
> — one writer — lives in
> [`HANDOFF-correction-revision-plan.md`](HANDOFF-correction-revision-plan.md)
> "The machine — program charter and spine"; it is not restated here.
