# AirPlay glitch investigation — archive (2026-05 … 2026-08)

> **Archived evidence, not current guidance.** The operational entry point is
> [HANDOFF-airplay.md](../HANDOFF-airplay.md); it owns today's patterns, shipped
> settings, and diagnostic recipes. This file keeps the measurements, root-cause
> derivations, source-cited shairport internals, and the dead ends behind them so
> a future debugger does not re-walk the same paths. Much of it describes the
> **retired dmix and snd-aloop topologies** — read every transport label
> literally.

## The receiver-side chirp proof, and why it is gone

Receiver-side chirp proof on JTS2 (2026-06-29) measured the local audio path at
roughly `84-87 ms` with a deliberate marker while the rendered offset was still
`-0.058667`. Retuning shairport to about `-0.0854` landed the measured receiver
delay within a few milliseconds once the Mac was actually sending program audio.
That number was never adopted as a constant — see
[ADR-0118](../adr/0118-the-airplay-latency-offset-is-derived-never-hand-set.md).

**The measurement is not reproducible, and the instrument is gone.**
`scripts/airplay-receiver-timing-proof.py` correlated the `plug:jasper_capture`
lane-7 summed-program tap against outputd's final electrical reference, and it
already refused outputd's production-default `shm_ring` content source — in the
ring topology lane 7 is a lossy diagnostic mirror rather than CamillaDSP's input
transport, so neither ring transport latency nor mirror drops were represented
by its model. The unified-audio deletion arc removed that tap, so the tool was
deleted with it rather than left as a script that cannot run on any surviving
topology. Even on the loopback coupling it only proved the receiver path from
the lane-7 tap to outputd's final electrical reference: it did **not** observe
the sender's display scanout, speaker acoustic output, or end-to-end A/V sync,
and small endpoint-open skew remained a hardware-validation uncertainty. A
ring-native receiver proof is not a port of that script; it needs direct ring
evidence first.

**2026-06-29 caveat that still stands:** audio-path counters being clean is not
the same as AirPlay video lip-sync being correct. JTS2 stayed clean at the
1024-frame fan-in output buffer, but video played from the computer over AirPlay
still had visible lip-sync problems. Do not reduce buffers further, or declare
the offset final for video, without a digital/acoustic A/V measurement or an
explicit Apple-side Wireless Audio Sync pass.

On the low-latency Apple-dongle DAC profile validated on JTS2 on 2026-06-29 the
live values were CamillaDSP `chunksize=256`, `target_level=1536`, fan-in output
buffer `1024`, and outputd DAC buffer `512`, so `jasper-apply-airplay-mode`
rendered `audio_backend_latency_offset_in_seconds = -0.058667`.
...
## The rate-adjust short-read storm — full mechanism
...
## Full polling diagnostic (loopback-era instrument)

This recipe polls the **snd-aloop** ring fill. It characterises Pattern
B/C behaviour on the historical loopback topology and has no SHM-ring
equivalent.
...
Run the [full polling diagnostic](#full-polling-diagnostic-loopback-era-instrument)
...
1. **Raise `resync_threshold_in_seconds` further** — try 0.3, 0.4, 0.5.
   ...
   discrete path never fires. Test with [full polling diagnostic](#full-polling-diagnostic-loopback-era-instrument).
...
## Pattern E variant — SETUP progresses, audio never starts

The layered diagnostic plan written for this open case. Layer 1 shipped
(`log_verbosity 1 → 2` in `deploy/shairport-sync.conf.template`); layers 2
...
explicitly as out of scope ([HANDOFF-resilience.md](../HANDOFF-resilience.md)).
...
Capture enough that the next reader can characterise a symptom that fits no
pattern, then add one to `HANDOFF-airplay.md` if it is worth preserving.

```sh
sudo journalctl --since "10 minutes ago" -o short-iso \
  -u shairport-sync -u jasper-camilla -u jasper-aec-bridge \
  -u jasper-voice -u nqptp -u librespot > /tmp/audio_journals.log
sudo dmesg --since "10 minutes ago" > /tmp/dmesg.log
aplay -l; arecord -l; free -m
cat /proc/asound/cards /etc/shairport-sync.conf /etc/asound.conf \
  /etc/modprobe.d/snd-aloop.conf > /tmp/alsa-state.txt
sudo cat /var/lib/camilladsp/outputd-statefile.yml \
  "$(sudo awk '/config_path:/ {print $2}' /var/lib/camilladsp/outputd-statefile.yml)" \
  > /tmp/camilla-state.txt
ps -eo pid,pri,ni,rtprio,policy,pcpu,pmem,comm | grep -E "shairport|camilla|jasper"
```
...
Archived 2026-08-26 from `docs/HANDOFF-airplay.md`. Dates and PR numbers
inside are as-written at the time of each investigation.
