# VAD / mic-stream test matrix (2026-05) — historical

> **Status: historical.** Frozen record of the 2026-05-24 session that
> diagnosed unreliable weather queries: three shipped bug fixes, the
> instrumentation added for the run, the five-cell VAD/mic-stream A/B matrix
> and its failure modes, the analysis recipes used to read it, and the
> raw-stream hypothesis that was left open. Kept because the matrix cost a
> recording session, because the cell recipes are how a sixth cell would be
> run, and because the raw-stream question is unresolved rather than closed.
>
> **Nothing here is current operational truth** — the live spine is
> [HANDOFF-vad-experiments.md](../HANDOFF-vad-experiments.md) and the decision
> is [ADR-0152](../adr/0152-local-silero-on-the-aec-stream-is-the-endpointer-server-vad-stays-off.md).
> Deployed state below reflects `build.txt` SHA `597f6df-dirty` on
> `jts2.local` in May 2026; paths, defaults and endpointer constants have
> moved since (`JASPER_MIC_DEVICE` now defaults to `Array`, and the
> sustained-speech arming window is 200 ms with an added peak gate, not the
> 240 ms recorded below). Verify against code before reusing any number.
> Relative links below were rewritten for this directory.
