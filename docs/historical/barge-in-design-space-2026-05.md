# Barge-in design space and option costing (2026-05-23)

> **Status: historical.** The original research snapshot costing the option
> space for robust barge-in under the earlier measure-first policy: Options A
> (software convergence sink), B (PipeWire), C (measure first / VAD-only), the
> alternatives considered, and the recommendation. Preserved for
> primary-source archaeology.
>
> **Still binding:** the standing rejection of AEC-topology re-architecture —
> no PipeWire echo-cancel, no snd-aloop replacement, no second reference route.
> That rule now lives with the topology it protects, in
> [HANDOFF-aec.md](../HANDOFF-aec.md).
>
> **Superseded:** every "today" / "current reality" snapshot below, including
> the warning against routing TTS through CamillaDSP — TTS now crosses
> CamillaDSP with program audio and reaches outputd's final-speaker reference
> safely. Current plan and gap analysis:
...
3. **Server-side auto VAD is structurally off.** All three
   ...
   manual-VAD path (see
   [historical/persistent-live-session-rework-2026-05.md](persistent-live-session-rework-2026-05.md)).
