...
| P6a–d | Renderer lanes → ring ingress (librespot, bluealsa, correction, shairport LAST) | **P6a UP, NOT MERGED** (branch `audio/u3-p6a-ring-ingress-librespot`, 3-lens panel owed) — the net-new build lands here: fan-in's `Input` gains a third lane source (`ring`) beside `lane` and `direct` (`rust/jasper-fanin/src/mixer/ring_capture.rs`), the librespot lane gets its ring PCM + `plug:` wrapper in `deploy/alsa/conf.d/61-jts-renderer-lanes.conf`, and the activation rule + its single writer live in `jasper/renderer_lanes.py`. **DEFAULT-EMPTY: no box is armed by this PR**, and the on-box source pass is OWED (see the U3 arc row). P6b–d are re-points through the seam this builds |
