...
| P6a–d | Renderer lanes → ring ingress (librespot, bluealsa, correction, shairport LAST) | **P6a MERGED 2026-08-13** (PR [#2389](https://github.com/jaspercurry/JTS/pull/2389), `b07c98c13`; three review rounds, all lenses 0/0). **P6b UP, NOT MERGED** (branch `audio/u3-p6b-ring-ingress-bluealsa`, panel owed) — the bluealsa lane, which is one registry row plus the three generalizations a second lane forced: the label is `bluealsa` (fan-in's vocabulary, not the mux source name `bluetooth`), `renderer_user_in_ring_group` had to learn that ROOT is capable without group membership (bluealsa-aplay runs as root, so the P6a preflight would have refused every arm of it), and the doctor's ring-device map became DERIVED from the registry instead of hand-listed. No `service-users.sh` change: root needs no `jts-ring`. Also default-empty and also owing its on-box source pass. The P6a net-new build lands here: fan-in's `Input` gains a third lane source (`ring`) beside `lane` and `direct` (`rust/jasper-fanin/src/mixer/ring_capture.rs`), the librespot lane gets its ring PCM + `plug:` wrapper in `deploy/alsa/conf.d/61-jts-renderer-lanes.conf`, and the activation rule + its single writer live in `jasper/renderer_lanes.py`. **DEFAULT-EMPTY: no box is armed by either PR**, and both on-box source passes are OWED (see the U3 arc row). P6c–d are re-points through the same seam; P6b measured the cost of the second one at one registry row plus three generalizations, with the Rust side needing no edit at all |
...
2. **Active/composite + bonded aloop dependencies survive naive
   ...
   still rides aloop lane 5, composite output has no ring at all until P8b,
   and bonded followers still write `hw:Loopback,0,6`. So P9 stays
   HARD-GATED on P8 on **both** halves: the capabilities P8b still owes
   (composite rings, bonded round-trip ingress) *and* the per-box arrival
   every remaining box has yet to make.
