...
| [#2332](https://github.com/jaspercurry/JTS/issues/2332) | Rollback ladder step 2 is refused on an ordinary armed box (ring-plan endpoint mismatch) | Open. PR #TBD (conductor: fill in at open) is **up, not merged** — documents the refuse-then-complete route as the canonical rollback path (owner-ruled 2026-08-12, this issue; validator untouched) and adds the matching doctor remediation text. The lifecycle section below has the route; hardware verification rides the pending jts3 corpus-exit session's de-arm ([owner ruling](https://github.com/jaspercurry/JTS/issues/2254#issuecomment-5267220207)) |
