...
| **jts.local** | Pi + single Apple dongle (card `A`), passive, USB-in box | **Narrow at its 2026-08-11 re-validation; the 2026-08-15 wire-default flip supersedes that half.** Installed `5dcd872a4`; coupling `shm_ring` + `shm_ring`; combo ARMED; the ring header stayed byte-identical `S16_LE` across that deploy and doctor was check-for-check identical pre/post, with ~5 min of real USB DIRECT playback at zero xruns / clips / drops (`captures/u2-jtslocal-regression-20260811T162805Z`). That build predates the flip. The box leaves `JASPER_FANIN_RING_WIRE_FORMAT` unset, so the first post-flip build it takes resolves `S32_LE` and it becomes a wide ring box without declaring anything — **not re-probed on hardware since**, so its ring header, its ioplug-capability verdict, and whether its graph needs a `baseline-reemit` are all owed. Still owed and unchanged by that pass: voice INACTIVE (parked; **commission owed**), the live dongle edge unconfirmed against the registry's `S24_3LE`, and the **route-latency revalidation artifact** (stale and failing both before and after) |
