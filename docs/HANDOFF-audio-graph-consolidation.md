...
| [#2345](https://github.com/jaspercurry/JTS/issues/2345) | fan-in emits `tts.assistant_loudness.final_gain_db=+3.0` while the doctor asserts the clamp is `[-60, 0]` — the assistant can be boosted past a bound the doctor believes is enforced **The doctor's assertion was the stale half**, so the question of whether music re-anchors it never had to be answered: the engine has had no fixed positive ceiling since "Remove fixed TTS gain ceiling" (2026-07-01), which updated `audio-paths.md` and `HANDOFF-volume.md` and missed the doctor. Its one fixed bound is the `-60 dB` floor; the `+3.0` was the peak cap (`max_peak_dbfs=-3.0` minus the uncalibrated fallback source peak `-6.0`) holding a `+5.0` request down, exactly as designed. Pending PR [#2355](https://github.com/jaspercurry/JTS/pull/2355) — the doctor now asserts the per-decision contract `max(floor, min(requested_gain_db, peak_cap_gain_db))` |
