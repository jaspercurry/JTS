...
| **P7** (U4) | fan-in's `RingOutput` keeps a lossy aloop MIRROR on lane 7 — which is *why* the dsnoop consumer survives ring coupling, and why the re-point may follow the default flip but must precede snd-aloop removal. Doctor pins to rewrite: `check_loopback` (`doctor/audio.py`), `check_fanin_asound_wiring` (`doctor/audio_runtime.py`), `check_shairport_sync_loopback_plughw` (`doctor/renderers.py`). **Both ALSA reference readers are gone**: the AEC bridge's `REF_DEVICE = "jasper_ref"` at P7-1, and `aec_tune`'s `arecord -D jasper_capture` at P7-2 — so CamillaDSP is the tap's last consumer, and on a ring-coupled box the mirror now has no in-tree reader at all. P7-4 removes the mirror itself |
