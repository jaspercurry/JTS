...
- **Decision:** the charge is the compiled stage's evaluated peak. Each chain
  (front, bass, cancellation) is evaluated as the complex response of its
  gain, polarity, delay and filters on `branch_chain.CHAIN_GRID_HZ`; the two
  rear branches are summed as complex numbers; the charge is
  `max(0, front peak, rear sum peak)`. A muted chain contributes nothing and
  `rear_muted` silences the sum. One evaluator does the filter maths
  (`branch_chain.camilla_filter_response`, over the shared RBJ biquad
  coefficients `jasper.sound.profile._biquad_coeffs`): an RBJ `Allpass` is
  `2·Notch − 1` over that same denominator, and a `Butterworth*` combo is
  `branch_chain.butterworth_response`, which a Linkwitz-Riley section already
  cascades twice. The chain-gain ≤ 0 dB bound and the
...
- **Consequences:** a cardioid speaker keeps the ~5 dB of program the old
  ...
  STEADY-TONE bound on a 1/48-octave grid: overshoot between grid points and
  transient overshoot stay backstopped by the per-output soft-clip limiter,
  as they already are for the linearization charge. The whole-sample rounding
