...
- **Decision:**
  1. A driver pose's first play, with no level asked, is its level probe (`compose_level_probe`,
     ...
     30 dB under the seat-equivalent level to the take's own ceiling. Each burst is one
     phase-closing step longer than the last, so no two share a shape. The whole probe is then one
...
- **Consequences:** Every driver pose costs its probe (7–10 s, less when the bound stops it) and
  ...
  reading's floor, and the gain it solved. Not decided here: the bass level ladder, a deliberate
