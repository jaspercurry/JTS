# ADR-0273: A graph naming the retired snd-aloop lane is refused, not preserved
...
- **Context:** ADR-0262 kept one retired spelling alive in production:
  ...
  `deploy/camilladsp/outputd-cutover.yml` before #3172 replaced that
  spelling; every emitter since writes a source marker that classifies
  before the device name is consulted. `deploy/install.sh` runs
  `jasper-sound render-flat-cutover`, the
...
- **Consequences:** No retired lane spelling survives in a production
  classifier branch; the two forbidden-token lists
  (`FORBIDDEN_ACTIVE_PLAYBACK_TOKENS` in
  `jasper/active_speaker/camilla_yaml.py`, `FORBIDDEN_TEST_PCM_TOKENS` in
  `jasper/active_speaker/playback.py`) keep it as a refusal. The
