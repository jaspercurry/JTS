...
Read `alignment_verdict.saved` and its `verification` line. The timing take plays the front drivers only (no bass layer, rear muted), the sum the driver takes predict. A `status` of `not_comparable` lists its `reasons`: `snr_short` names driver takes below the alignment SNR floor, `graph_mismatch` names outputs the take played that the prediction leaves out. Such a reading never asks for reset; `next_action` asks for a new measurement instead. A `comparable` `residual_rms_db` asks for reset only when it exceeds both three times `repeat_noise_db` and the `residual_floor_db` value of 0.5 dB. Act only on `next_action`; verification never changes the saved value. Pose rows disclose `margin_db`, `residual_rms_db`, `repeat_spread_db`, `repeat_spread_us`, and `repeat_count` (paired driver takes used). A missing spread means the read cannot establish confidence. See [ADR-0319](adr/0319-timing-measured-once-with-confidence.md).
