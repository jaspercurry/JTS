...
its `candidates.json`: each pose has pairwise deltas per role and window from
the banked frequency view. `level_offset_db` is median A minus median B on A's grid;
...
`roles_required`, `roles_per_take` counts, and `poses`. Separate driver takes
