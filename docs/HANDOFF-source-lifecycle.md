...
helper may wait up to 1443 seconds, just beyond the
coordinator's finite 1433-second systemd ceiling. Every other broker unit/verb
...
client's requested number alone. The coordinator's 1433-second ceiling has an
...
unit's own, plus the start ceilings of any unit it both orders `After=` and
pulls with `Requires=`/`Wants=`, because a synchronous start waits for the whole
job transaction rather than the named service alone — so the client cannot time
out while PID 1 may legally continue the job. AirPlay gets a 30-second start
ceiling for its root pre-start renderer, and its client also includes the
required NQPTP cold-start ceiling plus margin
...
`/bluetooth/` use `proxy_read_timeout 3000s` in the full and streambox nginx
configs: the stale-join case can require two broker calls at 1443 seconds plus
...
unbounded; the 1433-second coordinator and 60/120-second owner limits remain the
enforcement points.
...
- **Boot:** `jasper-source-intent-reconcile.service` is enabled for both install
  ...
  `TimeoutStartSec=1433`. It deliberately has no automatic `Restart=` loop;
...
- **Deploy:** both profile installers refresh only renderers that were already
  ...
  fact before waiting, drains the canonical reconcile lock for at most 1438
  seconds, and removes the fact again under that lock before applying. The
  outer 1443-second process ceiling deliberately leaves little room for a fresh
