...
- **Reconciler `reset-failed`s before deliberate recovery starts and config
  ...
  `TimeoutStartSec=6346`: its two-unit Snapcast plan, one-time provisioning
  (apt update + install), six bounded post-plan service/hardware actions, up to
  two 2698-second source starts, manager probes, and a terminal margin. It never
...
monitor — is untouched, still a separate reference consumer) ✓; inv-5
(CamillaDSP-B is the post-snapclient member DSP, `rate_adjust=false` —
shipped) ✓; `volume_limit:0.0` is applied by
