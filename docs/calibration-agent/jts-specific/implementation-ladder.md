...
| A3 | Channel/path safety | Started 2026-06-01 with `jasper.active_speaker.path_safety` and `jasper-active-speaker path-audit`: deterministic requirements/evidence evaluation only. Expanded 2026-06-02 with read-only `jasper-active-speaker environment-probe` for ALSA playback-device discovery, current/provided CamillaDSP config classification, volume-limit/channel inspection, optional `camilladsp --check`, and an explicit `safe_playback` block that keeps tone playback unauthorized. Added `jasper.active_speaker.safe_playback` no-audio arm/stop session state for the future tone path. Operator evidence can pass the checklist but cannot authorize active config loading. Still needs systemd/source-routing probes, hardware-probe-backed path evidence, physical output verification before drivers are connected, and a level-limited emergency-stop tone path before any playback. |
