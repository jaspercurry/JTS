...
| A3 | Channel/path safety | Started 2026-06-01 with `jasper.active_speaker.path_safety` and `jasper-active-speaker path-audit`: deterministic requirements/evidence evaluation only. Operator evidence can pass the checklist but cannot authorize active config loading. Still needs real ALSA/systemd/CamillaDSP/source-routing probes and physical output verification before drivers are connected. |
