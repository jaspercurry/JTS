...
A reconciler-managed XVF3800 is a commissioned chip-AEC product, not a generic
microphone. Supported hardware uses the single fixed profile owned by
`jasper/mics/xvf3800.py`. When the chip path cannot be armed — unsupported
firmware, output identity that moved, missing commissioning, a failed silent
reapply — the box **keeps hearing on software AEC3 (or the chip's plain capture
below 6 channels) and publishes `disclosed_stale` with the reason and the
recommission action**; it does not park (ADR-0101). Voice parks only for
`unavailable` (no XVF present) and `fault`. What a managed XVF never does is
fall back *silently*. `custom` remains the explicit low-level lab escape hatch,
...
[HANDOFF-aec.md](HANDOFF-aec.md) and at
...
self-heals it. `jasper-doctor` flags drift under "XVF mixer state".

The original pre-resolution ranking — the analytical process, and the failure
modes this root cause does not cover if ch2-5 ever go silent again for a
different reason — is preserved in
[historical/xvf3800-ch2-5-silence-investigation-2026-05.md](historical/xvf3800-ch2-5-silence-investigation-2026-05.md).
...
Last verified: 2026-08-26 (triage pass — the production AEC policy was
corrected: the ADR-0101 sweep replaced chip-or-park with chip-or-disclose, so
unsupported firmware, a moved output identity, missing commissioning, and a
failed silent reapply all run software AEC3 and disclose instead of parking
voice; parks remain for `unavailable` and `fault`. Re-read against
`jasper/cli/aec_init.py`, `jasper/audio_profile_state.py`, and the reconciler.
The ch2-5 pre-resolution ranking moved to `docs/historical/`.
Prior 2026-07-30 pass rechecked native reference geometry, the
foreground-commissioner-only volatile reset boundary, and the no-flash
lifecycle against `jasper/mics/xvf3800.py`. Prior 2026-06-30 pass rechecked
