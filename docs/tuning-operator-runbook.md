...
| `jasper-angle-capture plan\|stage\|show\|withdraw\|serve` | State one angle walk, see what it resolves to, leave it for the next measurement session, and serve it with the lab arm. | mutating (`stage` writes, `serve` moves the arm; `plan`/`withdraw` do not) | `jasper/cli/angle_capture.py` |
...
| `jasper-round-views entry\|frozen\|per-seat\|repeat\|repeat-floor\|candidates\|agreement\|co-metrics\|directivity\|cloud-binding\|forward-model\|spec-sweep\|gate-sweep\|frequency\|distortion\|classify-features\|close-reference\|delay-landscape\|delay-confirm\|inventory` | Grade, compare and disclose what one round measured — one subcommand per view, over banked rounds and live sessions, each answering on stdout and filing its artifact beside the round. | advisory (classify-features projects a ring into the bundle) | `jasper/cli/round_views/__init__.py` |
...
| `sigma_db_by_rung` | across-pose σ at this bin, per rung — the discriminator's raw material. `frame.uncertainty` says what kind of spread it is: `unseparated`, pooling the field's real pose-to-pose variation with each capture's own noise |
| `sigma_by_axis` | the same σ split by the axis the poses vary along: `azimuth` (every pose declaring elevation 0) and `elevation` (every pose declaring azimuth 0), the (0, 0) anchor in both. **Both axes are always present**, each carrying `sampled` and `n_poses` (distinct poses, not captures); a sampled one adds its own `sigma_db_by_rung`, `sigma_growth_ratio` and `sigma_growth_readable`, over the same valid rungs as the headline. An axis whose own angle took one value is `sampled: false` with `reason: axis_took_one_value`, and an undeclared pose field is never read as 0 — `reason: no_pose_declared_on_this_axis` — so an azimuth-only round says the elevation axis was **not measured** rather than leaving it to a missing key, and a round that mixes the two axes is the only one that can say a feature moves with HEIGHT (#3503) |
