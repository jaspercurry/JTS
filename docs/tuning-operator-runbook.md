...
| `jasper-round run\|trial\|placed\|stop\|status\|wait\|apply\|reset` | Run a plan across poses, graphs and levels, bank its packet, commission a speaker and apply candidates. | mutating-with-gates (`run`/`trial`/`placed`/`stop`/`wait`/`apply`/`reset` write; `status` reads) | `jasper/cli/round.py` |
