...
`nearfield/woofer`, `nearfield/rear` (a cardioid's rear woofer) and `nearfield/cardioid` (both, in turn) play one woofer with every other output silent. Start one from the measure page, which offers the rows a mono speaker declares, or stage it with `jasper-round run --program nearfield --poses nearfield/<size> --wait --timeout 3600` and send the link; a staged run lives in the web process until the first placement. Each woofer is taken at 15, 30 and 15 mm again, so the last take is a re-seat. For other distances pass a JSON pose list, for example `--poses '[{"azimuth_deg": 0, "elevation_deg": 0, "kind": "close", "distance_m": 0.02, "driver": "woofer:rear"}]'` (0 < distance ≤ 0.1 m from the dust-cap centre along the axis), and check the plan with `--dry-run` first. Turn off the fridge and the air conditioner, and put the capsule on the dust-cap axis with its tip level with a ruler laid across the surround. Each placement opens quiet and is retaken at the level that reads 80 dB at the microphone ([ADR-0355](adr/0355-a-near-field-take-levels-itself-to-80-db-at-the-microphone.md)). The takes are reference evidence that no tuning program reads ([ADR-0354](adr/0354-near-field-driver-takes-are-reference-evidence-one-driver-per-pose.md)); read them with `jasper-round-views nearfield <round>`.
...
| `jasper-round-views entry [speaker]\|repeat [all]\|candidates [all]\|directivity [speaker]\|sweep [all]\|impulse [all]\|group-delay [all]\|decay [all]\|compare [all]\|frequency [all]\|distortion [speaker]\|dsp-replay [all]\|dsp-levels [all]\|classify-features [speaker]\|close-reference [speaker]\|delay-landscape [speaker]\|room [room]\|room-grade [room]\|bass [bass]\|bass-compare [bass]\|bass-fit-table [bass]\|rear [rear]\|inventory [all]\|speaker-fit [speaker]\|nearfield [reference]` | Read measured round evidence, including off-axis directivity and mark-take repeat spread within and between rounds. Answers use stdout; detailed reports use files. | advisory (analysis views save artifacts) | `jasper/cli/round_views/__init__.py` |
