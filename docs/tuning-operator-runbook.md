...
4. Select evidence by set. Run `jasper-round-views <speaker-fit|room|bass-fit-table|repeat|sweep> <round-dir> --set <set-id>` as the question requires; use `inventory` for available sets and exact commands.
...
Use the `handoff_url`; the page is `/sound/speaker/crossover/` over HTTPS with the speaker's local CA, and banked rounds are at `/sound/measurements/`. First read the structured reason, run `jasper-doctor --json`, inspect `:8780/state`, and fetch logs with `bash scripts/fetch-pi-logs.sh`. Tool details and exit codes live in each tool's `--help`; the [methodology](tuning-methodology.md) and [doctrine](measurement-loop-doctrine.md) own science and authority rules.
