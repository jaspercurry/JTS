...
| `jasper-round run\|trial\|placed\|status\|wait\|apply` | Run a plan or bass level sequence, place the microphone, bank its packet, commission a speaker and apply candidates. | mutating-with-gates (`run`/`trial`/`placed`/`wait`/`apply` write; `status` reads) | `jasper/cli/round.py` |
