...
| `jasper-round run\|trial\|placed\|stop\|status\|wait\|apply` | Run a plan at --spl dB SPL or --levels fader dB, bank its packet, commission a speaker and apply candidates. | mutating-with-gates (`run`/`trial`/`placed`/`stop`/`wait`/`apply` write; `status` reads) | `jasper/cli/round.py` |
