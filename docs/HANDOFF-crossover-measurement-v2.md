...
as dated history — the disclosure line `level reference set fresh this
session — the previous one, <when>, differed by <x> dB`, rendered in the
verify screens' collapsed expert details on every outcome. It used to be
...
bench, 0.775 / 0.777 dB, deterministic to 0.002 dB). A session that MEASURES
drops the history rather than carrying it across an apply, since a pilot
transfer is captured through the applied graph. All thresholds are PROVISIONAL
...
| `verify_level_shift` | VERIFY | 2 | G3, session-scoped since #1927 — the recording chain moved DURING this sitting, so the capture is not evidence about the speaker. Same verify-fail screen, but its **copy names Re-measure / Undo, not the retry** (#1924): the in-session retry re-compares against the very reference it just failed against, and the screen's own "Try again" mints a session that re-baselines and so cannot reach this gate at all. Structurally unreachable on a session's FIRST usable attempt |
