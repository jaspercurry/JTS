...
proceeded arm, and it is the one to grep for when a session COMPLETED but the
two level estimators disagreed. It carries everything the refusal line does
plus `trim_band_average_db` (the other estimator) and `realized_difference_db`
(the check whose pass let the session proceed). The durable copy is the bundle's
...
`…_level_frame_finding_failed` (the store refused; the tune is unaffected) and
