...
| `measurement_volume_drift` | play seam | 0 (hard stop) | #2925: the main fader was not at the volume this session declared when a stimulus was about to play, and re-asserting it could not be proven, so the capture is refused before any audio. The OPPOSITE claim to `program_unplayable` — the program was admissible; the speaker's level was not the one it was admitted against — which is why it is its own code rather than another slug riding that one. Carries no refusal slugs: the observed/expected pair is on the `event=active_speaker.measurement_fader_drift result=refused` line. Terminal because a safety refusal is not something to retry around; the re-assert has already been tried and could not be confirmed. A drift the hold DOES repair is not a refusal — it plays, and says so at WARN (`result=repairing` then `result=repaired`) |
