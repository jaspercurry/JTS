...
`AUDIO_MGR_SYS_DELAY` is measured and persisted by `jasper-aec-commission`,
which writes the alignment artifact `jasper-aec-init` verifies at boot. (The
older `jasper-aec-tune` diagnostic estimated the same quantity by
cross-correlation and wrote it volatilely, so the next `jasper-aec-init` run
overwrote it; it was retired in favour of the commissioner.)

**Hardware verification still pending.** The Tier 2 follow-ups below
unlock once `jasper-aec-commission` produces a sane delay value AND
