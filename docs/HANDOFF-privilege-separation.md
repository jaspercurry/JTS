...
| jasper-control | `jasper-control` | `systemd-journal`, `jasper-intsecrets`, `jts-ring` | **3b-2 + 4b (LANDED)** | a **polkit rule** (broker/supervisor `systemctl`/reboot + a root `jasper-doctor-json` oneshot for /system/diagnostics), fresh HA/Spotify reads via `jasper-intsecrets`, group-readable non-secret config it reads off disk, `systemd-journal` for journal-based /state cards, and `jts-ring` (#2786: `/state`'s grouping `ring` block reads the grouping ring's 128-byte shared header — group `jts-ring`, mode per the creating unit's umask, 0660 or 0640, both group-readable). Control is the one member of `jts-ring` that never WRITES a ring — it opens `O_RDONLY`, pinned by `tests/test_grouping_ring_observability.py`. An external reader is unavoidable: unlike Ring A/B, whose fan-in and outputd owners publish what they read on their own STATUS surfaces, this ring's two ends are snapclient and CamillaDSP and neither publishes anything to aggregate. The grant is nonetheless WIDER THAN THE NEED — the group's purpose is ring write, and a file's group is its only handle — and that is a cost/complexity call, not an impossibility: a read-only default POSIX ACL on the ring directory (a tmpfiles `A+` line, which `deploy/tmpfiles/jts-ring.conf` could carry) would be narrower in capability, and was declined for having no ACL prior art in this repo, unverified tmpfs ACL behaviour on the Pi, and adding a second writer of permission state for one observability read |
...
TCP, a localhost CamillaDSP WebSocket, and the supplementary groups the table
above enumerates, which are not uniformly read-only: `jasper-intsecrets` carries
Spotify token-cache *writes*) with
...
| `jasper-wiim-remote-ce.service` + [`jasper/cli/wiim_remote_ce.py`](../jasper/cli/wiim_remote_ce.py) | every time the WiiM Remote 2 mic starts notifying — the non-root adapter `jasper-wiim-remote-mic` requests it on each reconnect | Root-only oneshot writes one raw `HCI_LE_Connection_Update` on the live remote link (CAP_NET_RAW; BlueZ hardcodes connection-event length to 0, which starves the mic to ~24 % of realtime on a Pi Zero 2 W) and verifies the controller's completion event. `jasper-input` may only `start` it through `START_ONLY_UNITS`. The helper takes **no argument naming a connection**: it enumerates live LE links itself and cross-checks each BD_ADDR against BlueZ, so the caller cannot aim it at an unrelated device. | Keep root. `CapabilityBoundingSet=CAP_NET_RAW` plus `RestrictAddressFamilies=AF_BLUETOOTH AF_UNIX` is the whole privileged surface; granting the adapter raw HCI instead would be strictly worse. |
...
| `jasper-audio-hardware-reconcile.service` + [`deploy/bin/jasper-audio-hardware-reconcile`](../deploy/bin/jasper-audio-hardware-reconcile) | install, boot, udev sound add/remove, and the start-only `/sound/setup/` I²S HAT toggle | Detect output hardware, reconcile the managed HAT setting in `/boot/firmware/config.txt`, write `/var/lib/jasper/outputd.env` and `/run/jasper-output-hardware/output_hardware.json`, render `/etc/jasper/asoundrc.jasper.template`, toggle `jasper-dac-init`/monitor, and kick outputd/AEC reconcile. | Split policy from root apply. Boot and `/etc/jasper/*` rendering plus unit orchestration should stay behind a fixed root helper; state observation can move to `jasper-recon`. |
...
privileged-root inventory updated for the composite
