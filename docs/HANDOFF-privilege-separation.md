...
read-access doctor check added 2026-06-21; the secret-compartment on-disk posture
doctor check + `voice_keys.env` re-tighten added the same day, hardware-free
tests only — on-Pi `jasper-doctor` run still owed; the always-on core-audio
daemons' root + `StartLimitAction=reboot` status verified on `jts.local` that day)
