...
| DA-0002 | `jasper/capture_relay/alignment.py` :: assert_alignment_confident | blocker | W0 | **partially-implemented** | the *property* is enforced on crossover (own locate floors — #1589/#1661, split #1838) and `/sync` (own `MIN_CONFIDENCE`, pre-audit); `room_sweep` unguarded; `bass_nearfield` has no production flow. This *function* still has no reachable production caller — #1384's lone call site is gated behind an `ambient_duration_s` no live path supplies. Nothing reads `validity.require_alignment`. Docs corrected #1214 + #1881; per-flow map + observe-first rollout tracked in #1882 — see "Open owner decisions" §1 |
...
1. **Relay alignment-gate wiring** (DA-0002, blocker — now **partially implemented**; re-verified against `main` 2026-07-29; remaining work tracked in #1882). The original recommendation still stands for the *unguarded* flows: per-flow seam, `require=False` + `event=capture_relay.alignment` logging first, flip `require=True` only after the 0.40 threshold is validated on jts3/jts5. Do NOT gate the shared `on_capture_uploaded` (same-origin uploads use it). What has changed since the audit:
   - **`crossover_sweep` is enforced** — by the flow's own locate floors in `active_speaker/crossover_v2_flow.py`, scored over the `cross_correlation_alignment` call `audio_measurement/program_analysis.py` makes in its own `_locate` (scoring added #1583). `_stimulus_locate_ok` (`LOCATE_MIN_CONFIDENCE = 0.1`) landed with the v2 conductor in #1589; `_sweep_locate_confidence_ok` (`SWEEP_LOCATE_CONFIDENCE_FLOOR = 0.3`) landed as a measurement-honesty gate in #1661 and was split off `_sweep_schedule_ok` by #1838's D3. Not via `assert_alignment_confident`, and not at the 0.40 default.
   ...
   - **One quantity, three implementations** (`capture_relay.alignment`, `program_analysis._locate` + flow floors, `sync_measure`) carrying four disagreeing thresholds (module default 0.40; crossover 0.3 and 0.1; sync 0.35). Consolidating is an SSOT decision worth making deliberately, not a side effect of wiring the next flow.
