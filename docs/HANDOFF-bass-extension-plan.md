...
target/candidate activation owner, and measured-context builder (#1630). Not
yet built: that runner's live on-device pre/post-limiter tap executor — its
CLI fails closed rather than pretend to measure, and binding it is blocked on
a tap-realization contract amendment. Still blocked: profile publication and
every production caller. The protocol freezes the detector point and units,
the minimum campaign, the accepted bundle shape, and total refusal; a real
...
the pure commissioning state machine + web backend module (new file, thin
