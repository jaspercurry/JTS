...
| S | 4627–5279 | 653 | 333 | CHECK · MEASURE · lateral · cloud-position verdicts and their retention. `_measure_verdict` (173), `_cloud_position_verdict` (77), `_retain_cloud_position` (65), `_consume_lateral_pose` (78) | internal; `_run_cloud_pipeline` reached from `capture_relay/session.py` | `W1-c` (the four retention sites) + `W2-b` (the verdicts) |
...
| X | 6729–6913 | 185 | 90 | Cloud pipeline runner (`_run_cloud_pipeline`, 160) + its diagnostic, with retention inlined at :6879 | `capture_relay/session.py`, web host | `W2-b` + `W1-c` |
...
| AB | 7925–8355 | 431 | 225 | Delta probe: `_run_delta_probe` (238), `_entry_delta_db` (134), offset/transfer helpers | internal; `delta_probe` property → `durable_state.py` | `W2-b` |
...
- **`W2-b` carries 40.5% of the file** — 3,734 lines across eleven bands. It is
  the largest single obligation on this file by a factor of two, and §8 tiers it
  as a default single review pass. That is worth a second look.
