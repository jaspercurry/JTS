...
addressed by PR #2473, which downgrades the verify verdict to `surface`
and discloses the reason when the verify capture's own SNR was low or
could not be estimated, with a dedicated household-register headline for
the downgraded case).
