...
all-pass) with a seat objective in place of a cardioid target, per unit front drive:
  ...
  - rear at least 20 dB down above 1 kHz, and a soft pull on the rear/front ratio to +6 dB or less
    below 900 Hz (each chain's gain stays inside fit-rear-branches.py's bounds).
...
jasper-crossover-prescriber (README.md has the steps).
"""
...
fitrb = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(fitrb)
...
def main() -> None:
    ...
    def residuals(p):
        r = fitrb._model(np.asarray(p, float), grid, False)
        at_seat = seat(r)
    ...
    cardioid_delay = fitrb.target_delay_ms(-cab.A["front"][:, behind] / cab.A["rear"][:, behind], grid)
    lower, upper = np.array(fitrb.PARAM_LOWER[:7]), np.array(fitrb.PARAM_UPPER[:7])
    ...
    document = read_rear_calibration(fitrb.build_document(
        best, allpass=False,
        ...
        sample_rate=fitrb.DEFAULT_SAMPLE_RATE)
    ...
    r_new = summed / front
    if not np.allclose(r_new, fitrb._model(best, grid, False), rtol=1e-3, atol=1e-5):
    ...
    args.out.write_text(json.dumps(prescription, indent=2) + "\n")
    print(f"wrote {args.out}; headroom charge {rear_branch_sum_headroom_db(document):.2f} dB")

    designs = {"front woofer only": 0 * grid, "new design": r_new}
    if args.live:
        designs["live"] = rear_ratio(args.live, grid)[0]
    ...
    print(" f Hz | new r dB / deg | seat vs front woofer only: " + " / ".join(n for n in designs if n != "front woofer only"))
    for f in TABLE_HZ:
        i = int(np.argmin(np.abs(grid - f)))
        print(f"{f:5d} | {db(r_new)[i]:+6.1f} {np.degrees(np.angle(r_new[i])):+5.0f} | " + " / ".join(
            f"{seat(r)[i] - front_only[i]:+5.1f}" for n, r in designs.items() if n != "front woofer only"))
    for label, kwargs in [(f"listener {m} m", {"listener_m": m}) for m in (1.5, 2.5, 3.0)] + \
        ...
        print(f"robustness, {label}: roughness " + ", ".join(
            f"{n} {roughness_db(seat(r, **kwargs), grid):.2f} dB" for n, r in designs.items() if n != "front woofer only"))
