...

{
    ...
    echo "#"
    echo "# One narrow exception, for the cross-ecosystem pin chain only (#1275):"
    echo "#   python3 scripts/align-pi-constraint-pins.py"
    echo "# rewrites just those pins to match uv.lock, which is what a uv-side"
    echo "# dependency bump needs to land. Everything else still comes from a Pi."
    echo "#"
    echo "# Provenance:"
