...
ACOUSTIC_EVERY_SECS="${S0_ACOUSTIC_EVERY_SECS:-300}"  # capture cadence in soak
MIC_DEVICE="${S0_MIC_DEVICE:-plughw:Array,0}"      # jts3 XVF array (plug: 6ch->1 downmix)

# Bench namespaces.
RUN_DIR="/run/jts-s0"
TMP_DIR="/tmp/jts-s0"
SNAPFIFO="${RUN_DIR}/snapfifo"
...
# the clock test). Width/format are detected at --up.
declare -A DAC_DEV=( [leader]="hw:sndrpihifiberry,0" [follower]="hw:A,0" )

# Transient unit names.
UNIT_SERVER="jts-s0-snapserver"
UNIT_FEEDER="jts-s0-feeder"
...
# ground truth). Identical L/R; 2 ms band-limited burst every 1.000 s, -20 dBFS,
# 48k/S16/stereo. Reuses scripts/multiroom-spike.sh's click rationale: a tone's
...
# -----------------------------------------------------------------------------
make_click() {
  log "[leader] generating 1 Hz broadband click track..."
  ssh_run leader "sudo install -d -m 0777 ${TMP_DIR}; bash -s" <<REMOTE
set -eu
out='${CLICK_WAV}'
if command -v sox >/dev/null 2>&1; then
  sox -n -r 48000 -c 2 -b 16 /tmp/_s0click.wav synth 0.002 noise gain -20 sinc 300-7000
  sox -n -r 48000 -c 2 -b 16 /tmp/_s0gap.wav synth 0.998 sine 0 gain -120
  : > /tmp/_s0concat.txt
  for i in \$(seq 1 60); do printf '/tmp/_s0click.wav\n/tmp/_s0gap.wav\n' >> /tmp/_s0concat.txt; done
  sox \$(cat /tmp/_s0concat.txt) "\$out"
  rm -f /tmp/_s0click.wav /tmp/_s0gap.wav /tmp/_s0concat.txt
else
  python3 - "\$out" <<'PY'
...
frames = bytearray()
for _ in range(reps):
    for s in click: frames += struct.pack('<hh', s, s)
    frames += gap
w = wave.open(out,'wb'); w.setnchannels(2); w.setsampwidth(2); w.setframerate(sr)
w.writeframes(bytes(frames)); w.close()
PY
fi
echo "click: \$out (\$(stat -c%s "\$out" 2>/dev/null || echo '?') bytes)"
REMOTE
}
  ...
  ssh_run "$hk" "aplay -D ${dev} --dump-hw-params -d 0.1 /dev/zero 2>&1 || true" \
    | awk '
      ...
      END {
        ...
        if(chn=="") chn=2
        # format: prefer S32_LE > S24_LE > S16_LE.
        f="S16LE"; if(fmt["S16_LE"]) f="S16LE"; if(fmt["S24_LE"]) f="S24LE3"; if(fmt["S32_LE"]) f="S32LE"
        print chn, f
      }'
}
...
# -----------------------------------------------------------------------------
# snapserver.conf — pipe source carrying the click; PCM codec (zero codec
# latency), buffer 1000 ms (generous WiFi jitter absorption; the spike already
# found PCM holds p99<5ms — this bench is about the snd-aloop seam, not codec).
# -----------------------------------------------------------------------------
write_server_conf() {
  ssh_run leader "sudo install -d -m 0777 ${TMP_DIR}; cat > ${SNAPSERVER_CONF}" <<EOF
# jts-s0 throwaway snapserver.conf. DO NOT ship.
[stream]
source = pipe://${SNAPFIFO}?name=jts-s0&sampleformat=48000:16:2&mode=read
codec = pcm
buffer = 1000
chunk_ms = 20
[tcp]
...
threads = -1
EOF
  log "[leader] snapserver.conf written (pcm/1000ms)"
}

# -----------------------------------------------------------------------------
# Stop / restore the live JTS audio stack (the bench needs exclusive DAC +
# snd-aloop ownership). Records which units were active so teardown restores
# exactly those.
# -----------------------------------------------------------------------------
stop_stack() {
  local hk="$1"
  log "[$hk] stopping live JTS audio stack (recording active set)..."
...
# Stop in dependency-friendly order (voice/mux first, DAC owner last).
sudo systemctl stop ${STACK_UNITS[*]} 2>/dev/null || true
echo "stopped; recorded active set:"; cat ${SAVED_UNITS} 2>/dev/null || true
REMOTE
}

restore_stack() {
  local hk="$1"
  log "[$hk] restoring live JTS audio stack..."
  ssh_run "$hk" "bash -s" <<REMOTE 2>/dev/null || true
set -eu
if [ -f ${SAVED_UNITS} ]; then
  # Restart the recorded set; systemd resolves ordering via their deps.
  units=\$(tr '\n' ' ' < ${SAVED_UNITS})
  [ -n "\$units" ] && sudo systemctl start \$units 2>/dev/null || true
  echo "restored: \$units"
else
  echo "no saved unit set; starting core audio units"
  sudo systemctl start jasper-camilla jasper-outputd 2>/dev/null || true
fi
REMOTE
}
...
# -----------------------------------------------------------------------------
start_server_feeder() {
  log "[leader] starting snapserver + click feeder..."
  ssh_run leader "bash -s" <<REMOTE
set -eu
sudo install -d -m 0777 ${RUN_DIR}
...
sudo rm -f ${SNAPFIFO}; mkfifo -m 0666 ${SNAPFIFO}
sudo systemd-run --collect --unit=${UNIT_SERVER} --property=Description='jts-s0 snapserver' \
  snapserver -c ${SNAPSERVER_CONF}
# Feeder loops the click into the FIFO forever (blocks until snapserver opens read end).
sudo systemd-run --collect --unit=${UNIT_FEEDER} --property=Description='jts-s0 feeder' \
  bash -c 'while true; do sox ${CLICK_WAV} -r 48000 -c 2 -b 16 -t raw - 2>/dev/null > ${SNAPFIFO} || true; done'
...
set +e
echo '--- units ---'
for u in ${UNIT_SERVER} ${UNIT_FEEDER} ${UNIT_CAMILLA} ${UNIT_CLIENT} ${UNIT_MONITOR}; do
...
do_smoke() {
  ...
  python3 "$(dirname "${BASH_SOURCE[0]}")/s0-sync-measure.py" acoustic --wav "$local_wav" || true
  do_status
}

# -----------------------------------------------------------------------------
# --soak HOURS : per-host monitor unit (xrun + budget hourly + periodic
# acoustic on the leader). Runs ON the Pi as a transient unit so it survives
# the operator's session ending. --collect pulls the accumulating logs.
# -----------------------------------------------------------------------------
do_soak() {
  local hours="${1:-${SOAK_HOURS}}"
  local max_sec=$(( hours*3600 + 600 ))
  log "==== starting ${hours}h soak monitors (xrun + budget + acoustic) ===="
  # Leader monitor: budget + xrun + periodic acoustic capture.
  ssh_run leader "bash -s" <<REMOTE
set -eu
sudo systemctl reset-failed ${UNIT_MONITOR}.service 2>/dev/null || true
sudo systemctl stop ${UNIT_MONITOR}.service 2>/dev/null || true
sudo install -d -m 0777 ${ACOUSTIC_DIR}
sudo systemd-run --collect --unit=${UNIT_MONITOR} --property=RuntimeMaxSec=${max_sec} \
  --property=Description='jts-s0 soak monitor (leader)' \
  bash -c '
    log='${SOAK_LOG}'; : > "\$log"
    last_acoustic=0
    while true; do
      now=\$(date -u +%s)
      xr=\$(journalctl -u ${UNIT_CAMILLA}.service --no-pager -o cat 2>/dev/null | grep -icE "xrun|underrun|overrun|buffer underflow|capture error" || true)
      ...
      cpss=\$(awk "/^Pss:/{s+=\\\$2} END{print s}" /proc/\$cpid/smaps_rollup 2>/dev/null || echo 0)
      spss=\$(awk "/^Pss:/{s+=\\\$2} END{print s}" /proc/\$spid/smaps_rollup 2>/dev/null || echo 0)
      printf "ts=%s host=leader camilla_xruns=%s temp_c=%s throttled=%s load1=%s camilla_pss_kb=%s snapclient_pss_kb=%s\n" \
        "\$now" "\$xr" "\$temp" "\$throt" "\$load" "\$cpss" "\$spss" >> "\$log"
      if [ \$(( now - last_acoustic )) -ge ${ACOUSTIC_EVERY_SECS} ]; then
        wav='${ACOUSTIC_DIR}'/cap-\$now.wav
        arecord -D ${MIC_DEVICE} -d ${ACOUSTIC_CAP_SECS} -f S16_LE -r 48000 -c 1 "\$wav" >/dev/null 2>&1 || true
        last_acoustic=\$now
      fi
      sleep 60
    done'
echo "leader monitor up (max ${max_sec}s)"
REMOTE
  # Follower monitor: budget + xrun only (no mic).
  ssh_run follower "bash -s" <<REMOTE
set -eu
sudo systemctl reset-failed ${UNIT_MONITOR}.service 2>/dev/null || true
sudo systemctl stop ${UNIT_MONITOR}.service 2>/dev/null || true
sudo install -d -m 0777 ${TMP_DIR}
sudo systemd-run --collect --unit=${UNIT_MONITOR} --property=RuntimeMaxSec=${max_sec} \
  --property=Description='jts-s0 soak monitor (follower)' \
  bash -c '
    log='${SOAK_LOG}'; : > "\$log"
    while true; do
      now=\$(date -u +%s)
      ...
      cpss=\$(awk "/^Pss:/{s+=\\\$2} END{print s}" /proc/\$cpid/smaps_rollup 2>/dev/null || echo 0)
      spss=\$(awk "/^Pss:/{s+=\\\$2} END{print s}" /proc/\$spid/smaps_rollup 2>/dev/null || echo 0)
      printf "ts=%s host=follower camilla_xruns=%s temp_c=%s throttled=%s load1=%s camilla_pss_kb=%s snapclient_pss_kb=%s\n" \
        "\$now" "\$xr" "\$temp" "\$throt" "\$load" "\$cpss" "\$spss" >> "\$log"
      sleep 60
    done'
echo "follower monitor up (max ${max_sec}s)"
REMOTE
...
sudo systemctl stop ${UNIT_MONITOR}.service ${UNIT_CLIENT}.service ${UNIT_CAMILLA}.service \
  ${UNIT_FEEDER}.service ${UNIT_SERVER}.service 2>/dev/null || true
