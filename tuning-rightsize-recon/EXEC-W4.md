# Execution brief — tuning toolbox, WAVE 4 (read fully before touching anything)
...
- `python3` has no numpy. Use the shared venv for anything that imports
  ...
  (`-m pytest`). `ruff` and `mypy` are at `/root/.local/bin/`.
...
- From /home/user/JTS: `git fetch origin main && git worktree add "$(mktemp -d)/wt" -b claude/tuning-rightsize/w4-<slug> origin/main`.
