...
# Upper bound at 500 Hz (the plan §7.3 sweep's own f2) so the full 150-400 Hz
# mic-fixity band (MIC_FIXITY_BAND_HZ) is exercised, not just its first half.
FREQS = np.geomspace(12.0, 500.0, 500)
...
# Fixture cabinet/manifest carried by every session built through _session(),
# mirroring what validate_commissioning_manifest would resolve from _MANIFEST
# below (kept in sync manually since _session() bypasses manifest parsing).
_SESSION_CABINET = CabinetInfo(
...
def _session(state=LadderState.IDLE) -> LadderSession:
    return replace(
        LadderSession(
            session_id="s", margin="normal", adapter_id="sealed_v1",
            cabinet=_SESSION_CABINET, n_targets=5, mic_calibration_id=None,
        ),
        state=state,
    )
...
def test_run_ladder_all_pass_is_incomplete():
    # B3 invariant 1: nothing ever failed, so no rung "stops tracking
    # cleanly" (plan §7.4) -- the type must not present a listening_level as
    # a proven ceiling when the ladder is refused as incomplete.
    ...
    assert ceiling.listening_level is None
...
def test_run_ladder_wraps_compression_curve_value_error():
    # Out-of-order commanded levels make compression_curve raise a bare
    # ValueError; run_ladder must wrap it in the module's own LadderError
    # rather than leak an untyped exception to callers (nit).
    with pytest.raises(LadderError):
        run_ladder([_measure(0, 3.0, 40), _measure(1, 0.0, 30)], NORMAL)
...
def test_apply_sustain_never_erases_an_existing_refusal():
    # B3 invariant 3: an all-pass ladder's ceiling already carries a refusal
    # (nothing proven -- see test_run_ladder_all_pass_is_incomplete). A
    # subsequent sustain failure must not synthesize a fresh, unrefused
    # ceiling: there is no rung to fall back to, since none was ever proven.
    records, ceiling = run_ladder([_measure(0, 0.0, 30), _measure(1, 3.0, 40)], NORMAL)
    assert ceiling.refusal is BassExtensionRefusal.LADDER_INCOMPLETE
    assert ceiling.listening_level is None

    failing_sustain = sustain_result([0.0, 0.0], [-2.0, 0.0], 60.0, 60.0, NORMAL)
    assert failing_sustain.verdict == "lowered"

    result = apply_sustain(records, ceiling, failing_sustain)
    assert result == ceiling
    assert result.refusal is BassExtensionRefusal.LADDER_INCOMPLETE
    assert result.listening_level is None


# --------------------------------------------------------------------------- #
# Shared target-family fixture (interpolate_anchors itself is covered by
# test_bass_extension_targets.py; ladder.py calls it directly -- see S1)
# --------------------------------------------------------------------------- #
...
def test_synthetic_dry_run_session_carries_the_full_manifest():
    # S1: start_session's logic was folded into synthetic_dry_run's caller
    # because it silently dropped cabinet/n_targets/mic_calibration_id --
    # only margin/adapter_id survived onto the session before this fix.
    session = synthetic_dry_run(_dry_run_inputs())
    assert session.cabinet == CabinetInfo(**_MANIFEST["cabinet"])
    assert session.n_targets == 5
    assert session.mic_calibration_id is None


def test_ladder_never_derives_a_limiter_threshold():
    # S2: the prior version of this test only asserted the injected
    # fixture's own limiter_threshold_dbfs=None -- tautological, since
    # TargetSpec is frozen and _family() never sets anything else, so it
    # proved nothing about ladder.py's own code. Prove the code itself:
    # no call anywhere in ladder.py passes a limiter_threshold_dbfs
    # keyword (mirrors the source-scan style of test_no_wave3_transaction_
    # references below). Keep the behavioral check as a second layer.
    for node in ast.walk(_LADDER_TREE):
        if isinstance(node, ast.keyword) and node.arg == "limiter_threshold_dbfs":
            assert False, "ladder.py must never pass a limiter_threshold_dbfs keyword"
...
def test_synthetic_dry_run_aborts_on_all_pass_ladder():
    # B3 invariant 2: a refused ladder must end in the aborted arm with the
    # refusal recorded in session.refusals -- it must NOT reach review with
    # publishable anchors for a ceiling nothing proved (invariant 4).
    session = synthetic_dry_run(
        _dry_run_inputs(rung_measurements=(_measure(0, 0.0, 30), _measure(1, 3.0, 40)))
    )
    assert session.state == LadderState.ABORTED
    assert session.refusals == (BassExtensionRefusal.LADDER_INCOMPLETE.value,)
    assert session.anchors == ()
    assert session.ceiling.listening_level is None
...
def test_module_imports_are_pure_and_hardware_free():
    # S3: mirrors test_module_imports_are_pure_and_hardware_free in
    # test_bass_extension_limiter_evidence.py. Module-scope only (tree.body)
    # -- the one function-local import (produce_limiter_thresholds) is
    # authorized separately by test_producer_import_is_function_local_only
    # above and by the cross-file guard in
    # test_bass_extension_limiter_evidence.py.
    imports: set[str] = set()
    for node in _LADDER_TREE.body:
    ...
    # The exact-match set above already structurally excludes os, socket, and
    # subprocess (and, since os is never bound, os.environ/os.getenv) without
    # a substring scan's false-positive risk on ordinary words containing
    # "os". The one hardware/env-adjacent surface it cannot rule out on its
    # own is the builtin open(), which needs no import.
    for node in ast.walk(_LADDER_TREE):
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name) and node.func.id == "open":
            assert False, "ladder.py must never call the builtin open()"
