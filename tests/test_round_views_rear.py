...
def rear_round(tmp_path: Path, *, candidates=(BASE_CANDIDATE, _MUTED, _VARIANT),
    ...
    artifacts = inputs.session_dir / "evidence/v1/artifacts"
    groups = []
    for candidate in candidates:
        rows = []
        for degrees, repeat in [(0, index + 1) for index in range(repeats)] + [(-20, 1), (20, 1)]:
            take_id = f"{candidate}-{degrees}-{repeat}"
            record = {**source, "take_id": take_id, "position_id": take_id, "repeat": repeat,
                      ...
                      "curves": [{**source["curves"][0], "magnitude_db": _CURVES[candidate]}]}
            record.pop("seat_offset_m", None)
            path = directory / "positions" / f"{take_id}.json"
            path.write_text(json.dumps(record))
            rows.append((str(path.relative_to(artifacts)), record))
        group = manifest_set(rows, set_id=candidate)
...
def test_a_rear_round_packets_one_comparison_for_the_whole_batch(tmp_path, banked_candidates):
    ...
    assert {row["view"] for row in views if row["status"] == "written"} >= {"rear", "inventory"}
