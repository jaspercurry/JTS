...
def test_rust_workspace_owns_packages_resolution_and_release_profile() -> None:
    rust = ROOT / "rust"
    workspace = tomllib.loads((rust / "Cargo.toml").read_text())
    members = {path for pattern in workspace["workspace"]["members"] for path in rust.glob(pattern)}
    assert members == {manifest.parent for manifest in rust.glob("*/Cargo.toml")}
    assert list(rust.glob("**/Cargo.lock")) == [rust / "Cargo.lock"]
    assert workspace["workspace"]["resolver"] == "2"
    assert workspace["profile"]["release"] == {
        "opt-level": 3, "lto": "fat", "codegen-units": 1,
        "strip": "debuginfo", "panic": "abort",
    }
    package = workspace["workspace"]["package"]
    assert package["rust-version"] == "1.82"
    assert package["license"] == "Apache-2.0"
    assert package["repository"] == "https://github.com/jaspercurry/JTS"
    for member in members:
        manifest = tomllib.loads((member / "Cargo.toml").read_text())
        assert all(manifest["package"][key] == {"workspace": True} for key in package)
        alsa = manifest.get("dependencies", {}).get("alsa")
        if alsa is not None:
            assert alsa["workspace"] is True
    cargo_updates = [entry for entry in _dependabot()["updates"] if entry["package-ecosystem"] == "cargo"]
    assert len(cargo_updates) == 1
    assert cargo_updates[0]["directory"] == "/rust"
