...
def _assert_names_the_way_out(message: str) -> None:
    """The refusal must route the operator somewhere that still exists.

    #2285 P2 retired the de-arm this refusal used to prescribe. It said "run
    ``baseline-reemit --endpoint aloop``", which argparse now rejects outright
    (``choices=("ring",)``) — a remediation string naming a command that
    errors is worse than none, because the operator does the work of trying it.
    So both halves are asserted: the retired command is GONE, and the tracked
    path forward (#2412) is named in its place.
    """
    assert "--endpoint aloop" not in message, message
    assert "#2412" in message, message
    assert "BEFORE a speaker is armed" in message, message
...
async def test_driver_commissioning_refuses_the_ring_before_it_emits(
    ...
    _assert_names_the_way_out(message)
...
async def test_the_refusal_reaches_the_guarded_load_preflight(tmp_path, monkeypatch):
    ...
    _assert_names_the_way_out(refusal["message"])
