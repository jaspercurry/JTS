...
_ALLOWLIST = {
    # Deliberately different — KEEP. Its parent-directory fsync (now
    # routed through fsync_directory) is fail-soft: any error the helper
    # re-raises logs at DEBUG and the write still succeeds (pinned by
    # test_wifi_guardian_persistence's fsync-failure test), because a
    # degraded rename-durability beats failing the wizard's connect.
    # atomic_write_text(durable=True) re-raises those errors, so
    # converging would change this writer's observable failure mode.
