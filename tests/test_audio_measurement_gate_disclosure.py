...
def test_describe_gate_renders_an_unknown_entanglement_floor_as_unknown():
    """#3495's second floor at the surface. With no reflection measured and no
    geometry declared, the sentence must say the ROOM's floor is unknown --
    the state a reader is likeliest to mistake for a clean gate, since the
    trusted floor alone reads as "trustworthy above this line"."""
    ir = _bandlimited_ir(2500.0, 18000.0)
    _gated, fragment = gating.gate_impulse_response(ir, SR)
    ...
    assert "room-entanglement floor is UNKNOWN" in unknown
    assert "1000 Hz no window separates speaker from room" in declared
    assert "declared room geometry" in declared
