...
def test_a_staged_walks_declared_geometry_is_banked_with_the_session(
    monkeypatch, tmp_path, declared,
):
    """The household's tape measure reaches the session's own bundle (#3498).
    ...
    so the answer has to survive from the operator's ``stage`` to the banked
    evidence an offline reader opens. Nothing in the session computes from it.

    A walk nobody was asked about publishes NOTHING, rather than an artifact
    claiming a room measured to zero.
    """
    ...
    angle_spool.set_angle_request_spool_path_for_tests(tmp_path / "angle.json")
    try:
        angle_spool.stage_angle_request(
