...
def test_gate_on_acoustic_quality_is_gone():
    """Owner ruling S8: the second judge that downgraded evaluate_acceptance's
    own ACCEPT verdict is deleted, not merely unused. The verify capture's
    SNR is still disclosed (session.py folds it into the acceptance dict as
    ``verify_quality_warned`` / ``verify_quality_reason``; the envelope turns
    it into a nudge) — see test_correction_session.py's
    test_verify_accept_discloses_quality_warning_when_verify_snr_warned and
    test_correction_envelope.py's
    test_verify_quality_warned_nudge_does_not_change_verdict — but it never
    again overrides the verdict itself."""
    assert not hasattr(acceptance, "gate_on_acoustic_quality")
