...
def test_completeness_flags_are_never_true_without_a_named_evidence_lane(
    ...
    for measurements in ({}, _measurements(topology, tmp_path)):
        verification = _verification(measurements=measurements)
        if not verification:
            continue
        for flag, source, count in (
            ("driver_target_proof_complete", "driver_target_proof_source", "captured_driver_count"),
            ("summed_validation_complete", "summed_validation_source", "validated_summed_group_count"),
        ):
            if verification[flag]:
                assert verification[source] == "measurements"
                assert verification[count] > 0
            else:
                assert verification[source] == "missing"
