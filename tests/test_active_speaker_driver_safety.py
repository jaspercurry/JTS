...
def _jts3_manual_settings() -> dict:
    # Same shape as ``_manual_settings()`` (known-good against the code-policy
    # highpass floor for an undeclared driver style), plus JTS3's declared
    # sensitivities.
    return {
        "drivers": [
            {
...
def test_sensitivity_flows_through_to_confirmed_profile_and_stays_confirmed() -> None:
    """W6.5: ``sensitivity_db_2v83_1m`` is an optional declared-spec
    pass-through -- present in ``manual_settings``, it lands on the
    confirmed target and the profile still validates and re-confirms fresh
    (round-trips through :func:`evaluate_driver_safety_profile`), so it can
    serve as the "declared specs the household confirmed" input to the
    sensitivity-derived HF measurement ceiling.
    """
    topology = mono_output_topology(card_id=None)
    profile = build_driver_safety_profile(
        topology,
        manual_settings=_jts3_manual_settings(),
        driver_research=None,
        confirm=True,
        confirmed_at="2026-07-19T12:00:00Z",
    )
    targets_by_role = {t["role"]: t for t in profile["targets"]}
    assert targets_by_role["woofer"]["sensitivity_db_2v83_1m"] == 83.3
    assert targets_by_role["tweeter"]["sensitivity_db_2v83_1m"] == 108.5
...
def test_sensitivity_is_optional_and_absent_when_undeclared() -> None:
    # The existing driver-safety fixture never declares sensitivity -- it
    # must stay absent (not an empty/null placeholder) and never block
    # confirmation.
    topology = mono_output_topology(card_id=None)
    request = build_driver_research_request(
        topology,
    ...
    profile = draft["driver_safety_profile"]
    assert "sensitivity_db_2v83_1m" not in profile["targets"][0]
    assert "sensitivity_db_2v83_1m" not in profile["targets"][1]
