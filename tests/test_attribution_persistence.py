...
def test_a_finding_round_trips_through_the_real_evidence_store(tmp_path: Path) -> None:
    ...
    publish_finding_set(
        store, relay_session_id=RELAY, phase=PHASE, finding_set=built
    )
    reopened = read_finding_set(store, relay_session_id=RELAY, phase=PHASE)
...
def test_a_finding_may_not_silently_predecease_its_evidence(
    ...
    with pytest.raises(FindingEvidenceMissing, match="no longer produce"):
        read_finding_set(store, relay_session_id=RELAY, phase=PHASE)
    ...
    assert read_finding_set(
        store, relay_session_id=RELAY, phase=PHASE, verify_evidence=False
    ) is not None
...
def test_a_bundle_written_before_attribution_stays_readable(tmp_path: Path) -> None:
    ...
    assert read_finding_set(store, relay_session_id=RELAY, phase=PHASE) is None
...
def test_ran_and_found_nothing_is_not_the_same_as_nobody_looked(
    ...
    empty = read_finding_set(store, relay_session_id=RELAY, phase=PHASE)
...
def test_the_capture_ring_sidecar_gains_the_index_it_never_had(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """§6's four-store problem, at the store where it bit hardest.

    WO-0 found the operator capture ring carrying NEITHER the bundle id nor
    the relay session id — only an epoch-microsecond filename stamp — which is
    why the SHA-256 of the WAV bytes became the corpus's only reliable join,
    and why directory mtime misrouted (the ~08:27 session lives in one bundle
    while the bundle whose mtime IS 08:27 holds the previous evening's run).
    """
    ...
    v2host._maybe_retain_capture(
        ...
        analysis=None,
        bundle_session_id="7f54494228cc",
    )

    sidecars = sorted(dump.glob("*.json"))
    assert len(sidecars) == 1
    sidecar = json.loads(sidecars[0].read_text())
    identity = read_session_identity(sidecar)
    assert identity is not None
    assert identity.session_id == "7f54494228cc"
    # And the full digest rides alongside the 12-char browsing prefix: a
    # prefix is not a verifier.
    assert len(sidecar["wav_sha256"]) == 64
    assert sidecar["wav_sha256"].startswith(sidecar["wav_sha256_12"])


def test_an_unbound_capture_still_retains_rather_than_refusing(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Retention is operator forensics and must never become a gate. A caller
    with no bundle in scope writes a sidecar without the identity key, which
    reads as legacy — the same state every pre-WO-1 sidecar is in."""
    ...
    v2host._maybe_retain_capture(
        phase="check", result=None, wav=b"bytes", analysis=None
    )
...
def test_the_cloud_artifact_and_its_findings_share_one_identity(
    ...
    assert cloud_identity.aliases["relay_session_id"] == RELAY

    findings = read_finding_set(store, relay_session_id=RELAY, phase=PHASE)
