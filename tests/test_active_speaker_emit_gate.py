...
def test_native_bass_preserves_static_graph_on_solo_and_driver_domain(driver_domain, classification):
    ...
    assert payload["filters"]["as_woofer_baseline_limiter"]["parameters"] == {"clip_limit": -1.0, "soft_clip": True}
...
def test_native_bass_tamper_is_rejected_at_emit_and_runtime_boundaries(monkeypatch):
    preset = _preset("stereo", 2)
    descriptor = _dynamic_bass_descriptor()
    ...
    proof = classify_bass_extension_graph(
        ...
        applied_baseline_state={"recomposition_snapshot": {"bass_extension": descriptor}},
    )
...
def test_absent_bass_extension_preserves_ordinary_baseline_bytes(descriptor):
    preset = _preset("stereo", 2)
    ordinary = emit_active_speaker_baseline_config(preset, playback_device=ACTIVE_PCM)
