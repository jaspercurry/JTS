...
class _FakeHost:
    ...
    def ops(self) -> reconcile.ReconcileOps:
        usb_role = reconcile.UsbPortRoleState(
            board_model="test",
            board_topology=(
                "separate_host_ports"
...
def accessory_request_file(tmp_path, monkeypatch) -> Path:
    ...
    monkeypatch.setattr(
        accessory_reconcile,
        "DEFAULT_RECONCILE_REQUEST_FILE",
        str(request),
    )
    return request
...
def test_source_request_and_reconcile_locks_are_shared_group_writable(tmp_path):
    ...
    host = _FakeHost()
    assert reconcile.reconcile(env_path=path, ops=host.ops()) == 0
...
def test_status_invalidation_happens_under_lock_before_apply(tmp_path, monkeypatch):
    ...
    def fake_lock(*, env_path, timeout_sec):
        assert env_path == env
        assert timeout_sec == (units.RECONCILE_SYSTEMD_TIMEOUT_SECONDS + 5.0)
        events.append("lock")
        yield
    ...
    monkeypatch.setattr(reconcile, "source_reconcile_lock", fake_lock)
    monkeypatch.setattr(reconcile, "_reconcile_once", fake_reconcile_once)
...
def test_lock_timeout_invalidates_old_status(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(reconcile, "source_reconcile_lock", timeout_lock)
...
def test_blocking_unit_waits_match_owner_oneshot_timeouts(monkeypatch):
    ...
    assert reconcile._run_unit_action("librespot.service", "reset-failed") == (0, "")
    assert reconcile._run_unit_action("librespot.service", "start") == (0, "")
    assert reconcile._run_unit_action("shairport-sync.service", "start") == (
        0,
        "",
    )
    assert reconcile._run_unit_action("jasper-usbsink.service", "start") == (
        0,
        "",
    )
    assert reconcile._run_unit_action("jasper-usbgadget.service", "restart") == (
        0,
        "",
    )
    assert reconcile._run_unit_action("bt-agent.service", "stop") == (0, "")
    assert reconcile._run_unit_action(
        units._ACCESSORY_RECONCILE_UNIT,
        "start",
    ) == (0, "")
    assert reconcile._run_unit_action(
    ...
    assert reconcile._run_systemctl("librespot.service", False) == (0, "")
    assert calls == [
        ...
        (
            ["systemctl", "start", units._ACCESSORY_RECONCILE_UNIT],
            65.0,
        ),
        (
        ...
        (
            ...
            units._UNIT_ENABLEMENT_ACTION_TIMEOUT_SEC,
        ),
        (
...
def test_direct_and_systemd_reconcile_invocations_serialize_before_read(
    ...
    monkeypatch.setattr(reconcile, "_reconcile_once", reconcile_once)
    path = str(tmp_path / "intent.env")
...
def test_reconcile_rejects_unknown_prefixed_key_but_applies_valid_sources(
    ...
    with caplog.at_level("WARNING"):
        rc = reconcile.reconcile(env_path=env, ops=host.ops())
    assert rc == 1
...
def test_declared_intent_unit_selects_ordinary_systemd_applier(monkeypatch):
    ...
    lifecycle = replace(
        reconcile.local_source_lifecycle(Source.SPOTIFY),
...
def test_airplay_resets_failed_required_companion_before_start():
    ...
    assert reconcile._apply_source(Source.AIRPLAY, True, True, ops) == "on"
    assert host.calls == [
        ("enable", main),
        ("enable", required),
...
def test_desired_on_does_not_start_when_reset_does_not_clear_failed_state(
    ...
    with caplog.at_level("WARNING"):
        rc = reconcile.reconcile(env_path=env, ops=ops)
...
def test_usb_disable_recompose_failure_stops_gadget_before_disarming(tmp_path):
    ...
    assert reconcile.reconcile(env_path=env, ops=host.ops()) == 1
...
def test_converged_usb_off_still_repairs_persisted_coupling_state(tmp_path):
    ...
    host = _FakeHost()
    assert (
        reconcile.reconcile(env_path=str(tmp_path / "missing.env"), ops=host.ops()) == 0
    )
    host.calls.clear()
    env = _write(tmp_path, f"{_key(Source.AIRPLAY)}=disabled\n")
    assert reconcile.reconcile(env_path=env, ops=host.ops()) == 0
...
def test_bluetooth_toggle_delegates_optional_accessories_to_their_owner(
    ...
    assert not any(call[1] == units._ACCESSORY_RECONCILE_UNIT for call in host.calls)
...
def test_hard_block_fails_bluetooth_without_blocking_other_sources(
    ...
    with caplog.at_level("WARNING"):
        rc = reconcile.reconcile(env_path=str(tmp_path / "missing.env"), ops=host.ops())
    assert rc == 1
    assert host.active["shairport-sync.service"] is True
    assert host.active["librespot.service"] is True
...
def test_stable_source_reconcile_log_emitted_for_every_source(tmp_path, caplog):
    ...
    with caplog.at_level("INFO"):
        assert (
            reconcile.reconcile(env_path=str(tmp_path / "missing.env"), ops=host.ops())
            == 0
        )
...
def test_oversized_utf8_intent_is_rejected_by_bytes(tmp_path, caplog):
    ...
    host = _FakeHost()
    with caplog.at_level("WARNING"):
        rc = reconcile.reconcile(env_path=str(path), ops=host.ops())
    assert rc == 1
    assert host.calls == []
...
def test_invalid_utf8_intent_is_rejected_before_hardware_actions(tmp_path):
    ...
    host = _FakeHost()
    assert reconcile.reconcile(env_path=str(path), ops=host.ops()) == 1
    assert host.calls == []


def test_main_returns_reconcile_exit_code(tmp_path, monkeypatch):
    monkeypatch.setattr(reconcile, "reconcile", lambda **kwargs: 1)
    assert (
        reconcile.main(
            [
                "--status-path",
...
def test_mux_is_started_only_when_the_shared_verdict_allows(
    ...
    monkeypatch.setattr(
        ...
        lambda: {reconcile.SHARED_LABEL: (shared_allowed, None)},
    )
    monkeypatch.setattr(
        reconcile,
        "_run_unit_action",
...
def test_converge_sources_runs_fresh_pass_when_inactive(monkeypatch):
    ...
    assert (
        reconcile_mod._converge_sources_after_role(
            grouping_active=True, units_changed=False
        )
        is True
    )
    assert calls == [
