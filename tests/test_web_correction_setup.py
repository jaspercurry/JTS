...
def test_crossover_v2_refusal_is_logged_not_silent(monkeypatch, caplog):
    ...
    The refusal this bare box hits FIRST has moved twice — #2662 W2b (the relay
    origin is now required only when the session actually opens on the relay)
    and #3507 (this suite's saved topology is a subless passive main, which is
    now an ADMITTED shape, so the walk reaches the driver-declaration gate
    instead of stopping at "no active crossover"). The subject is unchanged:
