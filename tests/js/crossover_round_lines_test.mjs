...
const choice = {id: 'tournament/full', label: 'tournament/full', lines: ['server summary'], action};
let env = {capture: null, round_choices: [choice], round_default: choice.id, round_lines: []};
...
env = {...env, round_choices: [], round_lines: ['server progress'], capture: {status: 'awaiting_capture',
  round_pending: {mover: 'human', prompt: {}, actions: [{...action, id: 'retake'}]}}};
render(env);
...
assert.equal(elements.get('crossover-walk-action').children[0].textContent, action.label);
