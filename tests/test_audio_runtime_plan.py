...
def test_validate_outputd_env_reads_the_override_store(tmp_path):
    ...
    outputd_env.write_text(f"{OUTPUTD_DAC_BUFFER_KEY}=1536\n", encoding="utf-8")
...
def test_a_setting_is_never_attributed_to_the_grouping_env_layer(tmp_path):
    ...
    def period(**kw):
        return audio_plan.build_audio_runtime_plan(
            ...
            **kw,
        ).setting(audio_settings.OUTPUTD_PERIOD_KEY)

    alone = period()
    with_grouping = period(grouping_outputd_env={"JASPER_OUTPUTD_PERIOD_FRAMES": "999"})
...
def test_route_config_hash_includes_fanin_direct_period():
    ...
    assert (
        changed_plan.route_latency_identity()["fanin_direct_config"]["period_frames"]
        == 128
    )
...
_SOLO = _grouping_cfg()
_VALID_LEADER = _grouping_cfg(enabled=True, role="leader", channel="left", bond_id="b")
...
def test_runtime_plan_to_dict_exposes_topology_and_correction_latency_gate():
    ...
    assert payload["correction_latency_eligibility"]["minimum_phase_or_iir"] is True
...
def test_correction_latency_gate_reads_active_fir_metadata(tmp_path, config_kind):
    ...
    fir_dir.mkdir()
    (fir_dir / "linear.json").write_text(
        json.dumps(
            {
                "mode": "linear_phase",
...
def test_correction_latency_gate_allows_peq_and_minimum_phase(tmp_path):
    ...
    fir_dir = tmp_path / "fir"
    fir_dir.mkdir()
    (fir_dir / "minimum.json").write_text(
        json.dumps(
            {
                "mode": "minimum_phase",
    ...
    for root in roots:
        for path in root.rglob("*"):
            ...
            try:
                ...
                UnicodeDecodeError,
                OSError,
            ):  # pragma: no cover - binary/unreadable
                continue
            for key in removed:
                if key in text:
...
def test_the_low_latency_route_names_the_bond_not_a_bridge_to_set():
    ...
    plan = audio_plan.build_audio_runtime_plan(
        base_env={audio_settings.AUDIO_ROUTE_PROFILE_KEY: ROUTE_USB_LOW_LATENCY_48K},
        grouping_outputd_env={"JASPER_OUTPUTD_DAC_CONTENT_LANE": "1"},
    )

    assert plan.route_policy_reason_codes == (audio_plan.ROUTE_POLICY_BONDED_MEMBER,), (
    ...
    assert (
        audio_plan.ROUTE_POLICY_OUTPUTD_OFF_RING not in plan.route_policy_reason_codes
    )
...
def test_a_shared_topology_reader_reads_once_and_changes_no_verdict(monkeypatch):
    ...
    unshared = transport_coherence.transport_coherence_report(outputd_env=outputd_env)
    assert len(reads) == 1
