...
_REAL_GET_MEMBER_GROUPING_READINESS = rooms_peers._get_member_grouping_readiness
...
def test_patching_rooms_peers_intercepts_the_calls_rooms_setup_makes(monkeypatch):
    """Every peer helper rooms_setup reaches is resolved off rooms_peers at
    call time, so ``rooms_peers`` is the ONE patch point. A test patch that
    stopped intercepting would otherwise pass silently."""
    tree = ast.parse(
    ...
    reached = {
        ...
        and node.value.id == "rooms_peers"
    }
    assert reached
    for name in reached:
        assert hasattr(rooms_peers, name), name
        assert not hasattr(rooms_setup, name), name

    ran: list[str] = []

    def sentinel(name, result):
        def call(*_a, **_k):
            ran.append(name)
            return result
        return call
    ...
    monkeypatch.setattr(
        rooms_peers, "self_addresses", sentinel("self_addresses", set()),
    )
    monkeypatch.setattr(
        rooms_peers, "discover_speakers_cached",
        sentinel("discover_speakers_cached", [{"address": "192.168.1.9"}]),
    )
    monkeypatch.setattr(
        rooms_peers, "get_member_grouping",
        sentinel("get_member_grouping",
                 {"enabled": True, "role": "follower", "channel": "right",
                  "bond_id": "bond-1", "leader_addr": "jts.local"}),
    )
    # Reached only through the unpatched rooms_peers.fan_out_grouping, so
    # this also pins the module-internal resolution.
    monkeypatch.setattr(
        rooms_peers, "post_grouping_to_member",
        sentinel("post_grouping_to_member", (True, "HTTP 200")),
    )

    handler, _ = make_real_handler(
        rooms_setup._make_handler(), "/swap", body=b"{}", content_type=None,
    )
    handler.do_POST()

    assert handler.status == 200
    assert set(ran) == {
        "self_addresses", "discover_speakers_cached", "get_member_grouping",
        "post_grouping_to_member",
    }
...
def _patch_discovery(monkeypatch, *, speakers, grouping=None, airplay_fit=None,
    ...
    Address is NIC-derived (not identity), so self_addresses / self_address
    ...
    monkeypatch.setattr(rooms_peers, "self_addresses", lambda: set(self_addrs))
    monkeypatch.setattr(
        rooms_peers, "self_address",
    ...
    monkeypatch.setattr(rooms_peers, "discover_speakers_cached", lambda: list(speakers))
...
def test_get_root_does_not_open_a_socket_for_discovery(monkeypatch):
    ...
    monkeypatch.setattr(rooms_peers, "discover_speakers_cached", _boom)
    h = _get("/")
    assert h.status == 200
...
def test_rooms_json_carries_live_pair_balance_snapshot(monkeypatch):
    ...
    monkeypatch.setattr(
        rooms_peers, "get_member_grouping",
        lambda a, known=None, *, timeout=rooms_peers.CONTROL_HTTP_TIMEOUT_SEC: (
...
def test_rooms_json_balance_snapshot_uses_short_peer_read(monkeypatch):
    ...
    def fake_get_grouping(
        addr, known=None, *, timeout=rooms_peers.CONTROL_HTTP_TIMEOUT_SEC,
    ):
        timeouts.append(timeout)
        return dict(peer_g) if addr == "192.168.1.9" else None

    monkeypatch.setattr(rooms_peers, "get_member_grouping", fake_get_grouping)
...
def test_rooms_json_self_hostname_and_room_flow_from_identity(monkeypatch):
    ...
    monkeypatch.setattr(rooms_peers, "self_address", lambda known=None: "")
    ...
    monkeypatch.setattr(rooms_peers, "discover_speakers_cached", lambda: [])
...
def test_grouping_routes_reject_incomplete_json_before_state_or_control_mutation(
    ...
    monkeypatch.setattr(
        ...
        "fan_out_grouping",
        lambda targets, **_k: (
            effects.append("fan_out_grouping")
...
def test_hostname_label_strips_local_suffix_and_dot():
    assert rooms_peers._hostname_label("jts3.local.") == "jts3"
    assert rooms_peers._hostname_label("speaker.local") == "speaker"
    assert rooms_peers._hostname_label("") == ""
...
def test_discovery_cache_serves_within_ttl_without_rebrowsing(monkeypatch):
    ...
    monkeypatch.setattr(rooms_peers, "_discover_speakers", _counting_browse)
    rooms_peers._disc_cache.update(at=0.0, result=[])

    first = rooms_peers.discover_speakers_cached()
    second = rooms_peers.discover_speakers_cached()
    third = rooms_peers.discover_speakers_cached()
...
def test_discovery_cache_rebrowses_after_ttl_expiry(monkeypatch):
    ...
    rooms_peers._disc_cache["at"] -= rooms_peers.DISCOVERY_CACHE_TTL_SEC + 1
    rooms_peers.discover_speakers_cached()
...
def test_discovery_cache_empty_result_does_not_poison(monkeypatch):
    ...
    monkeypatch.setattr(rooms_peers, "_discover_speakers", _flaky_browse)
    rooms_peers._disc_cache.update(at=0.0, result=[])

    assert rooms_peers.discover_speakers_cached() == []
...
def _post_bond(body, *, csrf_ok=True, monkeypatch, member_results=None):
    ...
    monkeypatch.setattr(_common, "reject_csrf",
                        lambda h: h.send_response(403) or h.end_headers())
    monkeypatch.setattr(rooms_peers, "self_address", lambda known=None: "192.168.1.5")
    ...
    monkeypatch.setattr(rooms_peers, "leader_handle", lambda: "jts-living.local")
...
def test_post_bond_preflights_grouping_readiness_before_fanout(monkeypatch):
    ...
    monkeypatch.setattr(
        rooms_peers, "_get_member_grouping_readiness", fake_readiness,
    )
...
# ---- fan_out_grouping: concurrent, INPUT-ORDER-preserving fan-out ----
...
def test_fan_out_grouping_preserves_input_order_despite_slow_failing_member(monkeypatch):
    ...
    results = rooms_peers.fan_out_grouping(targets)
...
def test_grouping_set_success_detail_distinguishes_live_and_scheduled_apply():
    assert rooms_peers._grouping_set_success_detail(
        200,
    ...
    assert rooms_peers._grouping_set_success_detail(200, b"") == "HTTP 200"


def test_fan_out_grouping_empty_targets_is_empty_list():
    assert rooms_peers.fan_out_grouping([]) == []
...
# ---- map_peers: the one bounded-concurrency primitive ----


def test_map_peers_runs_concurrently_and_preserves_input_order():
    """map_peers runs items concurrently (wall-clock << sum of per-item
...
def test_map_peers_empty_is_empty_list():
    assert rooms_peers.map_peers(lambda x: x, []) == []
...
def test_map_peers_caps_worker_count():
    ...
    def run():
        done.append(rooms_peers.map_peers(fn, items))
...
# ---- get_member_grouping: GET a peer's /grouping behind the SSRF guard ----
...
def test_get_member_grouping_refuses_non_lan_and_non_ip_target(monkeypatch):
    ...
    assert rooms_peers.get_member_grouping("8.8.8.8") is None         # non-LAN
    assert rooms_peers.get_member_grouping("evil.example.com") is None  # non-IP
    assert rooms_peers.get_member_grouping("::1") is None
    assert rooms_peers.get_member_grouping("fd00::7") is None
...
def test_get_member_grouping_forwards_timeout_then_parses_domain(monkeypatch):
    ...
    monkeypatch.setattr(rooms_peers, "_get_remote_json_result", fake_get)
...
def test_get_member_grouping_unwraps_grouping_envelope(monkeypatch):
    ...
    key ({"grouping": {...}}; see test_control_server.py). get_member_grouping
    ...
    got = rooms_peers.get_member_grouping("192.168.1.9")
    assert got == inner
    assert [(c["base_url"], c["path"]) for c in calls] == [
...
def test_get_member_grouping_none_when_envelope_missing_or_null(monkeypatch):
    ...
    assert rooms_peers.get_member_grouping("192.168.1.9") is None
...
def test_post_swap_rollback_failure_is_surfaced(monkeypatch):
    ...
    monkeypatch.setattr(rooms_peers, "post_grouping_to_member", flaky_self)
...
def _post_trim(*, monkeypatch, body, self_grouping, speakers=(),
    ...
    monkeypatch.setattr(rooms_peers, "discover_speakers_cached",
                        lambda: list(speakers))
    monkeypatch.setattr(rooms_peers, "self_addresses", lambda: set())
    monkeypatch.setattr(rooms_peers, "get_member_grouping",
                        lambda a, known=None: (peer_grouping or {}).get(a))
    monkeypatch.setattr(rooms_peers, "post_grouping_to_member", _post)
    raw = json.dumps(body).encode()
...
def test_bond_create_records_roster_on_leader_and_clears_follower(monkeypatch):
    ...
    monkeypatch.setattr(rooms_peers, "post_grouping_to_member",
                        fake_member_post)
    monkeypatch.setattr(rooms_peers, "leader_handle", lambda: "jts.local")
