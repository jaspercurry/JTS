...
def _post_apply_proof(store, harness, operation, counter, *, verdict="blend_ok"):
    ...
    quality = store.publish_json_artifact(
        f"post-apply-test/{capture_id}/quality.json",
        {
            ...
            "raw_artifact_fingerprint": raw.fingerprint,
        },
    )
...
async def test_three_current_graph_captures_issue_receipt_without_graph_mutation(
    ...
    harness, _candidate, apply_result, state, _previous, port, _load = await _apply(
    ...
    assert counter == 3
    target_plan = harness.service._required_target_plan(
        harness.service._current(verify_child_evidence=False)
    )
    applied = AppliedCandidateProof.from_mapping(
        apply_result["applied_candidate_proof"]
    )
    ...
    retained = restarted_store.current_live_mutation(claimed)
    assert retained is not None
    restarted_status = CommissioningVerificationService(
        run=claimed,
    ...
    assert read_commissioning_room_authority(
        replace(harness.authority.topology, topology_id="different-speaker"),
        run_state_path=restarted_store.path,
        sessions_root=tmp_path / "sessions",
    )["allowed"] is False
...
async def test_usable_failed_repeats_persist_failure_and_keep_room_locked(
    ...
    assert results[-1]["status"] == "verification_failed"
    assert results[-1]["failure"]["failure_code"] == (
    ...
    retained = restarted_store.current_live_mutation(claimed)
    assert retained is not None
    restarted = CommissioningVerificationService(
        run=claimed,
        run_store=restarted_store,
        evidence_store=store,
        plan=harness.plan,
