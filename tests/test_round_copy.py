...
from jasper.active_speaker.round_copy import ROUND_LABELS, round_lines, coverage_lines
...
def test_manual_redo_and_ended_round_lines():
    ...
    assert ROUND_LABELS["run_ended"] == "This run has ended. Choose a pose set to start the next one."
...
def test_post_round_coverage_keeps_packet_words():
    ...
    assert coverage_lines(packet, manifest) == ["Measured: 1 take.",
        "Waived: 20° right, ear height.", "Unqualified band (woofer): below 250 Hz.",
