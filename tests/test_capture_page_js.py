...
def test_capture_page_version_contract_is_published_and_cache_busted():
    ...
    assert version == {
        ...
        "capture_page_build": "20260715.1",
    }
    assert "main.js?v=20260715-1" in index_html
    main_js = (_REPO / "capture-page/js/main.js").read_text(encoding="utf-8")
    ...
    assert 'from "./level-events.js?v=20260715-1"' in main_js
...
def test_capture_page_level_ramp_uses_meter_protocol_without_wav_upload():
    ...
    assert (
        'import { runLevelRampProtocol } from "./level-events.js?v=20260715-1"'
        in main_js
    )
