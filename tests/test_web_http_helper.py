...
``onSnapshot`` instead of opening a second fetch (the landing page's volume
safety-mute banner). That override only speeds up ``onSnapshot``: the
sublabel repaint stays on its own 20 s cadence regardless of how often the
underlying poll ticks, so a faster caller doesn't pay for DOM writes it
didn't ask for.
