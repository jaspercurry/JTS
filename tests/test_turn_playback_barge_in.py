...
def _tts(**kwargs) -> FakeTts:
    """Playout whose flush confirms, the way a healthy socket's does."""
    return FakeTts(flush_ack=_flush_ack(), **kwargs)


def _chunk_barge_tts(turn: _FakeTurn, **kwargs) -> FakeTts:
    """Trips a local barge-in during the first chunk write."""
    tripped: list[str] = []

    def barge(method: str) -> None:
        if method == "write_segment" and not tripped:
            tripped.append(method)
            turn.request_local_interrupt()

    return _tts(on_call=barge, **kwargs)


def _drain_barge_tts(turn: _FakeTurn) -> FakeTts:
    """Trips a local barge-in during the drain tail (after the last
    chunk) — the burst-delivery interrupt window."""

    async def barge() -> None:
        turn.request_local_interrupt()
        await asyncio.sleep(0.02)

    return _tts(on_drain=barge)
...
def test_response_started_observed_once_before_first_playout_write():
    ...
    async def response_started() -> None:
        write_counts.append(len(tts.writes))

    async def first_write() -> None:
        first_write_counts.append(len(tts.writes))
...
async def test_playback_reports_acceptance_and_stops_after_a_failed_write(mode):
    turn, tts, report = _FakeTurn(), _tts(), PlaybackReport()
    if mode == "empty":
        turn._chunks = [b""]
    first_write = []
    write_calls = 0
    ...
    async def accepted():
        first_write.append(write_calls)
    ...
    assert write_calls == (0 if mode == "empty" else 1)
...
def test_local_barge_in_drops_buffered_audio_no_replay():
    ...
    tts = _chunk_barge_tts(turn)  # trips the interrupt during the first write
...
async def test_interrupt_owns_gap_write_and_drain_helpers(phase):
    ...
    turn, tts = Turn(), Tts(flush_ack=_flush_ack(), on_drain=blocking_drain)
    ...
    try:
        ...
        assert len(tts.writes) == (2 if phase == "drain" else 1)
        assert tts.flush_calls == 1
        assert turn.seam_calls == [("cancel", "barge_in")]
    finally:
        playing.cancel()
...
def _run_watchdog(turn, clock, monkeypatch):
    ...
    return asyncio.wait_for(
        idle_watchdog(turn, _tts(), timeout=999.0, response_stall_timeout=1.0),
        timeout=5.0,
    )
