...
from jasper.voice import conversation_capture
...
def capture(tmp_path, monkeypatch):
    ...
    with closing(ConversationCapture(store=store, voice_provider="test")) as capture:
        yield capture


def test_record_is_gated_by_capture_env(capture, monkeypatch) -> None:
    monkeypatch.setenv(CAPTURE_ENABLED_ENV, "0")

    capture.record("hello", "hi", session_id=None, mic_muted=False)

    assert capture.store.recent(10) == []


def test_record_allows_metadata_only_rows(capture) -> None:
    capture.record(
        None,
        None,
...
def test_record_lazily_opens_store_after_capture_enabled(
    ...
    monkeypatch.setenv(CAPTURE_ENABLED_ENV, "1")
    monkeypatch.setenv(DB_PATH_ENV, str(db_path))
    with closing(ConversationCapture(store=None, voice_provider="test")) as capture:
        capture.record("hello", "hi", session_id=None, mic_muted=False)
...
def test_record_skips_while_mic_muted(capture) -> None:
    capture.record("hello", "hi", session_id=None, mic_muted=True)

    assert capture.store.recent(10) == []


def test_record_enforces_retention_max_rows(capture, monkeypatch) -> None:
    ...
    assert [row.user_text for row in capture.store.recent(10)] == ["third", "second"]


def test_record_enforces_retention_days(capture, monkeypatch) -> None:
    monkeypatch.setenv(RETENTION_DAYS_ENV, "1")
    assert capture.store.add(
        ConversationTurn(
            id="old",
    ...
    assert [row.user_text for row in capture.store.recent(10)] == ["new"]
