...
def test_summed_admission_proves_the_whole_graph_and_actual_audio(tmp_path, change, refusal):
    ...
    topology, profile, targets = _profile_and_targets(
        woofer_floor=500 if change == "declared_woofer_floor" else 100,
    ...
    if refusal is None:
        ...
        assert min(segment.band[0] for segment in admission.segments if segment.role == "tweeter") == 20
...
def _cardioid_solo_take(monkeypatch, target, *, rear_peak=None, stimulus_dbfs=0.0):
    ...
    if rear_peak is not None:
        next(target for target in safety["targets"] if target["target_fingerprint"] == targets["woofer:rear"])[
            "level_duration_limits"]["max_effective_peak_dbfs"] = rear_peak
    monkeypatch.setattr(design_draft, "load_design_draft", lambda **kw: {"driver_safety_profile": safety})
    monkeypatch.setattr(conductor_context, "ensure_crossover_preview_ready", lambda draft: None)
    monkeypatch.setattr(commission_wiring, "resolve_capture_preset", lambda topology: _rear_pair("mono")[0])
    context = conductor_context.resolve_conductor_context({
        "active": True, "targets": {"drivers": active_driver_targets(topology)},
    }, topology=topology)
...
def test_a_one_driver_take_is_composed_from_its_own_target_and_admitted(tmp_path, monkeypatch, target):
    ...
    assert {(segment.role, segment.channel) for segment in stimuli} == {(target, 0)}
    assert stimuli[0].segment_id == f"pilot_{target}_lo"
    sweeps = [segment for segment in stimuli if segment.kind == "sweep"]
    assert len(sweeps) == 3
...
def test_a_rear_take_is_refused_when_only_the_rear_ceiling_is_lowered(tmp_path, monkeypatch):
    ...
    next(target for target in safety["targets"] if target["target_fingerprint"] == context.role_targets["woofer:rear"])[
        "level_duration_limits"]["max_effective_peak_dbfs"] = peak - 6.0
...
def test_summed_room_band_uses_resolved_floor_without_adding_highpass(tmp_path, low_hz, highpass, role):
    topology, safety, targets = _profile_and_targets(
        woofer_floor=40, woofer_measurement_floor=60, max_sweep_duration_s=4,
        woofer_highpass=highpass if role == "woofer" else None,
    )
    if role == "tweeter" and highpass is None:
        next(target for target in safety["targets"] if target["role"] == role)["required_protection_filters"] = []
    applied = _applied_profile(topology)
    measurement = MeasurementGraphProfile(
        ActiveSpeakerPreset.from_mapping(applied["recomposition_snapshot"]["preset"]),
        topology, {"woofer": 0, "tweeter": 1}, ACTIVE_PCM,
    ...
    assert admission.allowed is (low_hz >= 20 and (role == "woofer" or highpass is not None)), admission.to_dict()
...
def test_summed_segment_refusal_codes_and_fields(tmp_path, caplog, failed):
    ...
    target["level_duration_limits"]["max_effective_peak_dbfs"] = -30 if "level" in failed else 0
    target["level_duration_limits"]["max_sweep_duration_s"] = 1 if "duration" in failed else 4
    wav = tmp_path / "refusal.wav"
    write_program_wav(wav, program)
    admission = readmit_summed_program_from_wav(
        program, wav, graph_yaml=graph, topology=topology,
        safety_profile=safety, role_targets=targets, session_volume_db=-20,
