...
def test_round_levels_and_bounds(tmp_path, monkeypatch, case):
    monkeypatch.setattr(ab, "load_applied_baseline_profile_state", lambda: None)
    ...
    if case in {"one", "unshared", "bad"}:
        ...
        if case == "bounds":
            assert [r["round_id"] for r in rounds] == [str(i) for i in range(29, 21, -1)]
            assert len(reads) == ab.MAX_FILES


def test_payload_sources(tmp_path, monkeypatch):
    monkeypatch.setattr(ab, "load_applied_baseline_profile_state", lambda: {
    ...
    assert set(payload) == {"applied_fingerprint", "apply_path", "volume_step_db", "level_band_hz", "rounds"}
    assert payload["applied_fingerprint"] == "a" * 64
    assert payload["apply_path"] == ab.APPLY_PATH
    assert payload["level_band_hz"] == [40, 16000]
