...
def test_parked_config_format_stays_pinned_regardless_of_playback_format():
    ...
    DEFAULT_PIPE_SINK_FORMAT — mirrors the bonded-leader pipe sink in
...
def test_parked_config_format_stays_pinned_after_the_default_rebinds(
    monkeypatch,
):
    """The parked graph's ``playback_format`` argument was removed (it was
    never accepted by the emitter's only production caller); this pins that
    the emitted format still tracks ``DEFAULT_PIPE_SINK_FORMAT``, not
