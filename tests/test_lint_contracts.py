...
MAX_LINES_BY_PATH = {
    ...
    # 2026-08-19 (capture slip guard): 7,275 -> 7,247, a CUT rather than a
    ...
    # model. Net -28: about -50 moved out, +22 back for the adapter's
    # sub-sample placement, the fourth `glitch_inputs` entry, the role-less
    # grouping note the type checker asked for, and the DriftEstimate
    # docstring gaining the one thing a reader of those two fields now has to
    # know -- they populate exactly when the gate fires, and their sign and
    # segment id are ambiguous at an even cut, so the magnitude is the part
    # to read.
    "jasper/audio_measurement/program_analysis.py": 7_247,
}
