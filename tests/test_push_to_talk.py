...
def test_at_the_shipped_default_the_hold_cap_beats_the_idle_watchdog():
    ...
    `idle_watchdog`'s pre-response timer is anchored at the last inbound
    server message (turn OPEN until one arrives) and fires at
    `JASPER_IDLE_TIMEOUT_SEC`. A quiet hold — the common shape — never
    moves that anchor. `_end_turn` then cancels `play_responses` BEFORE
