...
# shared one-shot primitive jasper.net.mdns.browse_once and keeps only the
...
def test_discover_sync_maps_browse_once_to_output_dict(monkeypatch):
    ...
    import jasper.net.mdns as mdns
...
def test_verify_ssl_reading_agrees_with_config(monkeypatch, value):
    ...
    assert ha_mod.verify_ssl_from_state(state) is Config.from_env().ha_verify_ssl
