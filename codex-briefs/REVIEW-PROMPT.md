# Self-review prompt — run before opening every PR

Usage: at the end of a work session (any brief in this directory, or any
substantive change), run this prompt against the session's work *before*
opening or finalizing the PR. Paste it verbatim, or tell the agent to follow
the prompt below the horizontal rule. The coordinator/reviewer agent uses the
same prompt when reviewing the PR, so passing it honestly here means passing
review.
