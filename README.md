...
| [docs/multiroom-pairing-reliability-plan.md](docs/multiroom-pairing-reliability-plan.md) | Audio / multi-room architects | **Rescued plan, not yet executed.** Verbatim investigation-and-delivery-plan record extracted from draft PR #1851's closed branch; not current operational truth — that remains in [HANDOFF-multiroom.md](docs/HANDOFF-multiroom.md), [HANDOFF-distributed-active.md](docs/HANDOFF-distributed-active.md), and [HANDOFF-source-lifecycle.md](docs/HANDOFF-source-lifecycle.md). Facts reflect a 2026-07-28 snapshot and need re-verification before execution; tracked by #1852. |
...
| [docs/HANDOFF-audio-graph-consolidation.md](docs/HANDOFF-audio-graph-consolidation.md) | Audio architects | **Campaign plan (unified).** The canonical plan for both halves of the audio program: consolidating the graph onto SHM rings + the `jts_ring` ioplug while deleting every duplicate/legacy path (snd-aloop, `transport_pipe`, `rate_match`), *and* preserving source-native precision to a single DAC-edge quantization. Current position per half, ratified decisions, U0–U4 sequencing over the P-rows, per-arc gates, risk register, done criteria; the 2026-07-03 no-dupes audit and default-flip narrative sit in its historical appendix |
...
- [`measurement-loop-doctrine.md`](docs/measurement-loop-doctrine.md) —
  **Canonical doctrine (owner-ratified 2026-08-21)** for how an LLM drives
  ...
  rules), the program's guiding principle — "least-bad measured, honed in
  bites", migrated here from `audio-commissioning-roadmap.md` in #2865 — and
...
- [`flat-linearization-flow-simplification-plan.md`](docs/flat-linearization-flow-simplification-plan.md)
  — **Design work order (2026-07-27)** for the flow-simplification phase after
  the first real cloud session: the express commission tier (7 captures,
  4 prompted positions with both wide offsets, no post-apply cloud, honestly
...
- [`audio-commissioning-roadmap.md`](docs/audio-commissioning-roadmap.md)
  ...
  carve-out until #2865 moved that ruling on 2026-08-23 into
  ...
  the audio
  commissioning program: the ethos rulings as originally written
  (tinker-first/never-nanny,
