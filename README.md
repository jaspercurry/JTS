...
| [docs/HANDOFF-runtime-memory.md](docs/HANDOFF-runtime-memory.md) | Maintainers / AI | **Active investigation + implementation handoff.** Live jts2 memory accounting for `jasper-voice` and `jasper-control`: voice RAM is dominated by resident openWakeWord/Silero instances, especially the three-detector chip-AEC wake profile; control's scary cgroup number mostly mixes real process PSS with reclaimable file cache and dashboard-route lazy imports. Includes the mergeable PR plan for tier-aware lean wake legs and honest dashboard memory reporting. |
