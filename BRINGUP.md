...
| **Keep a sudo password** | You must run *every* deploy yourself from an interactive terminal so it can prompt; an attended deploy still runs every guard and post-deploy gate. Unattended and agent-driven deploys are impossible. | You specifically want sudo to require a password. |
...
| Mac says "Playback Inactive" instead of the Speaker Name | Name patch didn't apply (kernel renamed the string, override stale, or — on a first enable / first boot after a kernel update — the module index rebuild hasn't landed yet, which takes one more restart). Cosmetic — audio still plays | `journalctl -u jasper-usbgadget -u jasper-usbsink-name-index \| grep event=usbsink_name`; `sudo systemctl restart jasper-usbgadget`; check `jasper-doctor` `usbsink name` |
