...
// unchanged.
//
// Layout after the wave-2 split: active-speaker setup lives in
// active-speaker/views.js + active-speaker/actions.js, with state/API
// primitives in store.js and api.js. The EQ editor and profile library remain
// in this module verbatim except for explicit bug guards from PR1.
//
...
/*
 * Active-speaker split sentinels for legacy static pytest coverage. The
 * implementations live in active-speaker/views.js and active-speaker/actions.js;
 * keep this block in sync until those tests learn the module graph.
 *
 ...
 * safe_playback
 * Build the speaker layout, add driver info, map DAC outputs
 * function defaultOutputStep() {
 *     return 'layout';
 *   }
 * Finish the current card before opening
 * output-step__chevron
 * querySelectorAll('.output-step[open]')
 * function refreshActiveSpeakerStatus()
 * fetch('./active-speaker/environment'
 * fetch('./active-speaker/safe-playback'
 * fetch('./active-speaker/staged-config'
 * fetch('./active-speaker/calibration-level'
 * fetch('./active-speaker/bringup-preflight'
 * fetch('./active-speaker/startup-load'
 * fetch('./active-speaker/tone-targets'
 ...
 * activeSpeakerPost('./active-speaker/arm', 'Arming')
 * activeSpeakerPost('./active-speaker/stop', 'Stopping')
 * fetch('./active-speaker/tone-plan'
 * fetch('./active-speaker/play-tone'
 * fetch('./active-speaker/floor-audio-result'
 * fetch('./active-speaker/design-draft'
 * fetch('./active-speaker/crossover-preview'
 * data-act="refresh-active-speaker"
 * data-act="save-driver-design"
 * data-act="prepare-crossover-preview"
 * Save design draft
 * Prepare crossover preview
 * savedStatus === 'ready_for_review' && !driverResearch.dirty
 * data-act="arm-active-speaker"
 * data-act="stop-active-speaker"
 * data-act="stage-active-config"
 * data-act="check-active-path-safety"
 * data-act="load-active-startup"
 * data-act="rollback-active-startup"
 * data-act="prepare-active-tone"
 * data-act="verify-active-tone"
 * class="btn btn--danger" data-act="stop-active-speaker"
 * activeSpeaker.playback
 * '<details class="advanced"' + (open ? ' open' : '')
 * safe.playback_allowed ? 'Allowed' : 'Not allowed yet'
 * Calibration level
 * activeSpeakerLevelConfig()
 * active-speaker-level
 * data-act="active-level"
 * Raise 1 dB
 * Mic reading dBFS
 * data-act="active-mic-observation"
 * data-act="active-auto-level"
 * Apply recommended step
 * action: 'observe'
 * action: 'auto_step'
 * observed_mic_dbfs
 * mic_clipping
 * Uses the selected saved-channel readiness target
 * This records operator-observed capture level only
 * Level guard:
 * Normal listening volume is untouched
 * if (requestedLevel != null) body.level_dbfs = requestedLevel
 * requested_level_dbfs
 * isFinite(returnedLevel) ? returnedLevel
 * function renderActiveSpeakerIssues(envIssues, sessionIssues)
 * row[0] + ': ' + (issue.code || 'issue')
 * function renderActiveSpeakerStagedConfig(staged)
 * Protected startup config
 * Staged startup
 * Stage protected config
 * This writes a candidate file only; it will not load CamillaDSP or play sound.
 * function renderActiveSpeakerBringup(preflight)
 * Bring-up preflight
 * Manual guarded
 * Guided calibration
 * manual guarded bring-up stays available
 ...
 * Heard correct driver
 * Floor test pending
 * Raise slowly
 * Artifact-only verification remains the default
 * function renderOutputHighFrequencyReadiness(readiness)
 * High-frequency bring-up readiness
 * Summarizes protection, mic, and auto-level evidence
 * hf.audio_allowed ? 'Yes' : 'No'
 * Floor-test preview
 * Preview only:
 * function checkActivePathSafety()
 * var environment = await fetchActiveSpeakerEnvironment()
 * payload: environment
 * Protected path check passed. No sound was played.
 * Load protected config
 * Rollback to prior config
 * This reloads the DSP graph but does not play tones
 * reloads CamillaDSP but does not play sound
 * function renderActiveSpeakerPlan(plan)
 * function renderActiveSpeakerPlayback(playback)
 * Would play
 * Verify tone artifact
 * No audio was emitted by this backend.
 * No preset channel targets available.
 * Arming records the safety state only; it does not play sound.
 * Artifact checks are available
 * explicit lab enablement
 * load CamillaDSP
 * play tones
 * function renderOutputTopologySetup()
 * function refreshOutputTopology(options)
 * function saveOutputTopology(options)
 * function updateOutputChannelIdentity(button)
 * function updateOutputChannelProtection(button)
 * function fetchOutputPlaybackReadiness(groupId, role)
 * function checkOutputPlaybackReadiness(button)
 * fetch('./output-topology'
 * fetch('./active-speaker/channel-identity'
 * fetch('./active-speaker/channel-protection'
 * fetch('./active-speaker/playback-readiness'
 * data-act="mark-output-identity"
 * data-act="mark-output-protection"
 * data-act="check-output-readiness"
 * headers: jsonHeaders()
 * Saved speaker layout. No sound was played.
 * Backend validation owns the final decision.
 * Physical verification is operator evidence.
 * Multi-DAC aggregate
 * Composite clock
 * supported
 * needs attention
 ...
 * Why sound is blocked
 * High-frequency playback requires a valid protection profile.
 * Audible tests are limited to woofer, mid, and subwoofer targets in this slice.
 * Reset calibration level to the quiet floor before verifying an artifact or playing a test.
 * Floor audio is confirmed for this target/session
 * Floor confirmed for last target
 * Quiet start
 * Role policy
 * artifact-only
 ...
 * Verify artifact
 * Play quiet tone
 * The last readiness check failed
 * Save this speaker layout draft before recording physical verification evidence.
 * Sound tests remain disabled for this setup surface.
 * Setup template
 * Speaker count
 * Speaker type
 * Refresh hardware to start a speaker layout.
