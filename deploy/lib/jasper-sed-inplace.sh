...
# This fragment is sourced, never executed. The shebang declares POSIX sh
# rather than the bash its deploy/lib siblings declare (so the static linter
# holds the body to sh): deploy/configure-bluez.sh sources it in sh dialect.
...
# root:jasper 0640. That backup is a second copy of a token-bearing file:
# it goes immediately.
sed_inplace() {
    _sed_inplace_file="$1"
    shift
