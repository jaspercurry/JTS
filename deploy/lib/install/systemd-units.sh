...
reapply_source_intent() {
    ...
    # process-level bound beyond the unit's 2753-second contract so a
...
PY
    ...
    # pass (bounded by the unit's 2753 s ceiling); a failed/timeout path removes
    # the file once more so no reader can mistake an older generation for this
    # install's.
    rm -f /run/jasper-source-intent/status.json
    if ! /usr/bin/timeout --foreground --kill-after=5s 2763s \
