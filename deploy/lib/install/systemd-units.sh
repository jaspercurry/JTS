...
reapply_source_intent() {
    ...
    # process-level bound beyond the unit's 2393-second contract so a
...
PY
    ...
    # pass (bounded by the unit's 2393 s ceiling); a failed/timeout path removes
    # the file once more so deploy health cannot accept an older generation.
    rm -f /run/jasper-source-intent/status.json
    if ! /usr/bin/timeout --foreground --kill-after=5s 2403s \
